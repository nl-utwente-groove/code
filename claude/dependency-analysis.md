...
| `grammar.Grammar`/`GrammarModel` → prolog | opaque `GrooveEnvironment` carrier on `Grammar`; `GrammarModel` loads `.pro` resources for validation | B | per-`ResourceKind` validator/environment-builder service | done (P2 branch: `Grammar` carries the active prolog program *texts* (`Map<QualName,String>`) instead of a built environment; `GrooveEnvironment.ofGrammar(GrammarModel)` / `ofGrammar(Grammar)` build the environment in the prolog layer; `.pro` validation at grammar build inverted via a `ResourceValidator` ServiceLoader service (`ResourceValidators` registry, prolog contributes `PrologValidator`; direct loading, no Provider indirection — validators are stateless). Cost note: the environment is no longer cached on the model, so the prolog display rebuilds it per grammar update — .pro parsing is cheap and off the exploration path) |
...
| `grammar.OracleParser` → transform.oracle | hard-coded `switch` over `ValueOracleKind` (the `dialogOracle` static is the one inverted case) | A/B | move to `transform.oracle`, convert switch to registry | done (P2 branch: moved; switch kept — the kinds are a closed enum, and the open case already goes through `setDialogOracle`) |
| `grammar.Rule`/`Prover` → match, transform | `Prover` near-duplicates `Rule`'s matcher-cache block; a stalled extraction | A | finish it: move `Prover` + matcher cache into `match` | done (on master directly, via the `prover-to-match` branch: `getProver` caching bug fixed, `Prover` → match, `Rule`'s duplicate matcher block deleted, `getMatchingTime` inlined into its sole caller `StatisticsReporter`. Residual grammar → match edge is the single `match.Prover` import in `Rule`; a full cut was rejected — it would need a static or `Grammar`-owned Prover cache, which leaks across grammar reloads resp. breaks standalone-rule use) |
| `grammar.host` → transform | delta vocabulary (`DeltaTarget`, `DeltaStore`, `DeltaApplier`, …) is a graph-mutation primitive | A | move delta types down to `grammar.host` (or `graph.delta`); `transform` keeps rule application only | done (P2 branch: all six delta classes moved to `grammar.host`; `graph.delta` rejected — the generic tier already exists as `graph.GDeltaTarget`) |
...
| `io.graph.AutIO` → explore.util | `ExplorationReporter.time()` stopwatch calls | A | fold into the gh #891 diagnostic-logging work | done (P2 branch: local `Log.getLogger("explore.timing")` logger in `AutIO` — same channel, no `explore` import) |
...
| `verify.CTLModelChecker` → explore | `Generator.LTSLabelsHandler` CLI reuse, plus `Generator.execute` for the `-g` option (undersold in the original inventory: the CLI *runs an exploration* before checking) | B | handler moved to `lts.LTSLabels.Handler` (not `util.cli` — that would create `util.cli`→lts); facades extracted to `verify.ModelFacade`, CLI tool moved to `explore.CTLModelChecker`; `BoundaryParser`'s `EnabledRuleParser` use fixed by moving that class to `grammar` | done (`verify` now has no upward edges) |
| `automaton` ↔ grammar | two tiers sharing a package: `RegExpr` syntax (belongs beside `grammar.rule`) vs host-graph evaluation (belongs beside `match`) | B/C | split, e.g. `match.regexpr` for the evaluation tier; residual `RuleLabel`→`RegAut` needs a factory hook | done (`automaton-split` branch: package dissolved — `RegExpr`+`RegExprCalculator` → `grammar.rule` (mutually recursive with `RuleLabel`/`LabelVar`), `RegExprTyper` → `grammar.type` (sole consumer `TypeGraph`), evaluation tier incl. `NodeRelation`/`RelationCalculator` → new `match.automaton`. The `RuleLabel`→`RegAut` residual was cut outright rather than hidden behind a factory hook: `RuleLabel.getAutomaton`'s per-label cache deleted, the three call sites compute via `RegAutCalculator.instance()` — all construction-time, no shared replacement cache (a static one would leak across grammar reloads). Residual intra-cluster edge: `grammar.RuleDependencies`/`grammar.model.RuleModel` → `match.automaton`) |
