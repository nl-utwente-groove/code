...
3. Partly done: Arend's `hub` grammar (2026-09-22) with the star row (5.6, 4.3.2) and
   ...
   state, so "states C(N,k)" needed the symmetry-breaking chain. Still missing: 5.1
   (mixed incident edges at the hub), 5.2 (a rule with an unconnected typed node) and
   3.7 (a `let:` per move), all on an unstored variant where matching is the whole
   per-step cost; design in the note's hub paragraph.
