 ...
 */
@NonNullByDefault
public class IsRuleApplicableCondition extends ExploreCondition<Prover> {
    /**
    ...
    public IsRuleApplicableCondition(Rule condition, boolean polarity) {
        super(condition.getProver(), polarity);
    }

    @Override
    public boolean test(GraphState state) {
        boolean result = criterion().hasProof(state.getGraph());
        return isNegated()
            ? !result
            : result;
    }
