...
/**
 * A <code>POptionalNumber</code> is a <code>SerializedParser</code> that reads a
 * positive number from a <code>StringConsumer</code>. If a number is found, it
 * is appended to an argument of a <code>Serialized</code>. If no number is found,
 ...
 */
@NonNullByDefault
public class POptionalNumber implements SerializedParser {
     ...
     */
    public POptionalNumber(String argumentName, Integer defaultValue) {
    ...
    public boolean parse(StringConsumer stream, Serialized serialized) {
        ...
        var value = foundNumber
            ? stream.getLastConsumed()
            : this.defaultValue;
        serialized.appendArgument(this.argumentName, value);
        return true;
    }
    ...
    public @Nullable String toParsableString(Serialized serialized) {
        ...
        if (result != null) {
            ...
            if (result.equals(this.defaultValue)) {
                result = "";
            }
        }
        return result;
    ...
    public boolean containsDefault(Serialized serialized) {
        return this.defaultValue.equals(serialized.getArgument(this.argumentName));
    }
