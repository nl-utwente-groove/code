...
/**
 * View model of an LTS: follows the GTS as it is explored, under a bound on the
 * number of states shown and the display options of its controller, and buffers
 * the changes during an exploration.
 * @author Arend Rensink
 * @version $Revision$
 */
@AIGenerated("Claude Fable 5.1, 2026-09")
@NonNullByDefault
public class LTSGraphViewModel extends GraphViewModel<GTS> implements GTSListener {
    /** Creates a new model for a given controller and cell store. */
    public LTSGraphViewModel(LTSGraphViewController controller, CellStore<GTS> store) {
    ...
    public synchronized void addUpdate(GTS gts, GraphState state) {
        if (isExploring()) {
            ...
            prepareInsert();
            // add a corresponding cell to the model
            addNode(state);
            doInsert(false);
        }
    }
    ...
    private boolean isAcceptState(GraphState state) {
        if (state.isInner() && !getController().isShowRecipeSteps()) {
            return false;
        }
        if (state.isAbsent() && !getController().isShowAbsentStates()) {
            return false;
        }
        return true;
    }
    ...
    protected boolean addEdges(@Nullable Collection<? extends Edge> edgeSet) {
        ...
        if (edgeSet == null) {
            ...
            for (Edge edge : edgeSet) {
                ...
                if (getController().getTransitionClass().admits(trans)) {
                    result |= addTransition(trans);
                }
            }
        }
    ...
    private boolean isAcceptTransition(GraphTransition trans) {
        return getController().getTransitionClass().admits(trans);
    }

    /**
     * Adds a given transition to the model, if its properties allow this,
        ...
        if (changed) {
            ...
            if (exploring) {
                ...
                if (!this.changedCells.isEmpty()) {
                    getCanvas().refresh(this.changedCells, true);
                }
            }
        }
    }
    ...
    /** Set of cells with status changes during the last exploration. */
    private final List<ViewCell<GTS>> changedCells = new ArrayList<>();
