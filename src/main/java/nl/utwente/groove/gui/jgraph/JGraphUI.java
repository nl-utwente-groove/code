...
import static nl.utwente.groove.gui.jgraph.JGraphUI.DragMode.SELECT;
...
public class JGraphUI<G extends @NonNull Graph> extends BasicGraphUI {
    ...
    protected void updateCachedPreferredSize() {
        ...
        if (in != null) {
            this.preferredSize
                .setSize(this.preferredSize.getWidth() + in.left + in.right,
    ...
    protected Point2D getEditorLocation(Object cell, Dimension2D editorSize, Point2D pt) {
        ...
        return super.getEditorLocation(cell, editorSize,
                                       new Point2D.Double(
                                           pt.getX() + scale * (EXTRA_BORDER_SPACE + 4) - 4,
    ...
    public class MouseHandler extends MouseAdapter implements Serializable {
                ...
                if (isEdgeAdding() && e.getClickCount() == 1) {
                    ...
                    if (selectedCell instanceof JEdge<?> selectedItem
                        ...
                        if (selectedEdge == jEdge) {
                            jGraph.getController().removePoint(selectedEdge, at);
                        } else {
                            jGraph.getController().addPoint(selectedEdge, at);
                        }
                    }
                } else if (getJCellAt(e.getPoint()) != null) {
                    // select and possibly start adding edge, or edit vertex
                    ViewCell<G> jCell = getJCellAt(e.getPoint());
         ...
         */
        private void selectCellsForEvent(Collection<ViewCell<G>> cells, MouseEvent evt) {
            List<ViewCell<G>> nonGrayCells = new LinkedList<>();
         ...
         */
        private ViewCell<G> getJCellAt(Point2D p) {
         ...
         */
        private ViewCell<G> getJVertexAt(Point2D p) {
         ...
         */
        private ViewCell<G> getJEdgeAt(Point2D p) {
