 ...
 */
public class VerbosityHandler implements ITypeConverter<Verbosity> {
    @Override
    public Verbosity convert(String value) {
        int result;
        try {
            result = Integer.parseInt(value);
        } catch (NumberFormatException e) {
            throw new TypeConversionException(String.format("\"%s\" is not a number", value));
        }
        if (result < Verbosity.LOWEST || result >= Verbosity.HIGHEST) {
            throw new TypeConversionException(String
                .format("\"%s\" is not a valid value for \"%s\"", value, NAME));
        }
