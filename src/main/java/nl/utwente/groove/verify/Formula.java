...
public class Formula extends ATermTree<LogicOp,Formula> {
    ...
    /** Checks if the propositions in the formula can be generated by the given grammar. */
    public void check(Grammar grammar) throws FormatException {
        computeErrors(grammar).throwException();
    }

    /** Checks if the propositions in the formula can be generated by the given grammar,
     * and returns a (possibly empty) set of errors found in this check. */
    private FormatErrorSet computeErrors(Grammar grammar) {
        ...
        if (prop == null) {
            ...
            switch (getOp().getArity()) {
            case 1:
                result.addAll(getArg1().computeErrors(grammar));
                break;
            case 2:
                result.addAll(getArg1().computeErrors(grammar));
                result.addAll(getArg2().computeErrors(grammar));
                break;
            default: // do nothing
            }
        } else {
            result = prop.computeErrors(grammar);
        }
        return result;
    }
