...
class NewExploreData {
    ...
    private Set<RecipeTarget> computeForwOuter() {
        ...
        Queue<NewExploreData> queue = new LinkedList<>();
        queue.add(this);
        while (!queue.isEmpty()) {
            NewExploreData source = queue.poll();
    ...
    private Set<NewExploreData> backInner;

    /** The forward reachable inner states. */
    private Set<NewExploreData> forwInner;
    ...
    private Set<NewExploreData> backTransient;

    /** The forward reachable transient states. */
    private Set<NewExploreData> forwTransient;

    /** The forward reachable transient open states. */
    private Set<NewExploreData> forwTransientOpen;
    ...
    void notifyOutPartial(RuleTransition partial) {
        ...
        NewExploreData targetData = target.getCache().getExploreData();
        ...
        if (target.isTransient()) {
            var succForwTransient = new HashSet<>(targetData.forwTransient);
            for (var back : this.backTransient) {
                back.forwTransient.addAll(succForwTransient);
                back.forwTransientOpen.addAll(targetData.forwTransientOpen);
            }
            for (var forw : succForwTransient) {
    ...
    void notifyClosure() {
        ...
        if (!getState().isTransient()) {
            removeFromForwTransientOf(this);
            this.backTransient.forEach(this::removeFromForwTransientOf);
        }
    }

    /** Remove this record from the {@link #forwTransientOpen} of a given state data. */
    private void removeFromForwTransientOf(NewExploreData data) {
    ...
    final void notifyTransience(int transience) {
        ...
        if (this.inner && !getState().getActualFrame().isInner()) {
            this.inner = false;
            // add incoming recipe transitions
            var gts = getState().getGTS();
            var target = new RecipeTarget(getState());
            for (var back : this.backOuter) {
        ...
        setAbsence(getState().getActualFrame().getTransience());
    }
    ...
    private void setAbsence(int newAbsence) {
        if (newAbsence < getAbsence()) {
            for (var back : this.backTransient) {
                back.getState().setAbsence(newAbsence);
            }
        }
    }
    ...
    static private final Set<NewExploreData> EMPTY_SET = Collections.emptySet();
