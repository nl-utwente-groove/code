...
public class WildcardEdgePathChecker extends SingleEdgePathChecker {
    ...
    public void receive(ReteNetworkNode source, HostEdge gEdge, Action action) {
        if (!isGuarded() || getGuard().test(gEdge.getType())) {
    ...
    public boolean equals(ReteNetworkNode node) {
        return (this == node) || ((node instanceof WildcardEdgePathChecker) && this.getOwner()
            .equals(node.getOwner())
            && this.expression.equals(((WildcardEdgePathChecker) node).getExpression()));
    }
