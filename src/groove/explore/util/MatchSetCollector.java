...
public class MatchSetCollector {
     ...
     */
    public MatchSetCollector(GraphState state, ExploreCache cache, SystemRecord record, Collection<VirtualEvent.GraphState> virtualEventSet) {
        this.state = state;
        this.cache = cache;
        this.record = record;
    ...
    public RuleEvent getMatch() {
        ...
        while (result == null && currentRule != null) {
            ...
            if (virtualEvents == null) {
                ...
                Iterator<RuleMatch> matchIter = currentRule.getMatchIter(state.getGraph(), null);
                if (matchIter.hasNext()) {
                    // convert the match to an event
                    result = record.getEvent(matchIter.next());
                } else {
    ...
    /**
     * Returns the set of matching events for the state passed in by the constructor.
     */
    ...
    /**
     * Collects the set of matching events for the state passed in by the constructor
     * into a collection passed in as a parameter.
     */
    ...
    private boolean collectEvents(Rule rule, Collection<RuleEvent> result) {
		...
		if (enabledRules == null || enabledRules.contains(rule)) {
    ...
    private Collection<RuleEvent> getVirtualEvents(Rule rule) {
        ...
        if (virtualEventMap == null && virtualEventSet != null) {
            virtualEventMap = computeVirtualEventMap();
        }
        if (virtualEventMap == null) {
            return null;
        } else {
            return virtualEventMap.get(rule);
        }
    }
	...
	private Map<Rule, Collection<RuleEvent>> computeVirtualEventMap() {
        ...
        if (virtualEventSet != null) {
			for (VirtualEvent.GraphState virtual : virtualEventSet) {
				Rule rule = virtual.getRule();
				if (!disabledRules.contains(rule) || virtual.hasMatch(state.getGraph())) {
    ...
    private Rule firstRule() {
        ...
        if (result == null && cache.hasNext()) {
            // this means that the cache was freshly created and has never been incremented before
        	result = cache.next();
        }
        return result;
    }
    ...
    private static final Reporter reporter = Reporter.register(MatchSetCollector.class);
    ...
    private static final int COMPUTE_EVENT_MAP = reporter.newMethod("computeVirtualEventMap");
}
