...
public class Validator {
    ...
    public static void main(String[] args) {
        for (String element : args) {
            if (element.startsWith(OPTION_PREFIX)) {
                processOption(element.substring(OPTION_PREFIX.length()));
            } else {
        ...
        if (!helpGiven) {
            ...
            if (verbosity > GraphFileHandler.QUIET_MODE) {
                System.out.println("Number of graphs validated: "
                    + (graphsValidated - rulesValidated));
                System.out.println("Number of rules validated: "
                    + rulesValidated);
            }
    ...
    static private void printHelpText() {
        ...
        for (String element : OPTIONS) {
            System.out.print("[" + OPTION_PREFIX + element + "] ");
        }
        System.out.println("[files]");
        System.out.println("Options: ");
        for (int i = 0; i < OPTIONS.length; i++) {
            System.out.println("  " + OPTION_PREFIX + OPTIONS[i] + "  "
        ...
        for (ExtensionFilter element : FILTERS) {
    ...
    static private class FileValidator extends GraphFileHandler {
        /**
         * Constructs an instance of the file validator with given verbosity
         * level.
         */
    ...
    static GraphShape validateGraph(File file) {
        try {
            graphsValidated++;
            String name =
                verbosity == GraphFileHandler.VERBOSE_MODE ? file.getName()
                        : file.toString();
            if (verbosity > GraphFileHandler.QUIET_MODE) {
    ...
    static void validateRule(File file) {
        ...
        try {
            ...
            if (verbosity > GraphFileHandler.QUIET_MODE) {
                ...
                System.out.println("Rule format error in " + file + ": "
                    + exc.getMessage());
            }
        }
    }
