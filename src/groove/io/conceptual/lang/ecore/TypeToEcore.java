...
public class TypeToEcore extends TypeExporter<EObject> {
    ...
    public void addTypeModel(TypeModel typeModel) throws PortException {
        ...
        Resource typeResource = m_ecoreResource.getTypeResource(typeModel.getName());
        EList<EObject> contents = typeResource.getContents();

        for (EPackage pkg : m_rootPackages) {
    ...
    public String getTupleName(Tuple tuple) {
        return m_currentTypeModel.getTupleName(tuple);
    }
    ...
    private EPackage packageFromId(Id id) {
        ...
        if (id == Id.ROOT) {
            ...
            m_packages.put(id, idPackage);
            addMessage(new Message("A package (ROOT) was generated for the root namespace, please check your identifiers", MessageType.WARN));
            m_rootPackages.add(idPackage);
            return idPackage;
        }
    ...
    public void visit(Field field, Object param) {
        ...
        if (fieldType instanceof Container) {
            ...
            if (cmContainer.getContainerType() == ContainerType.SET || cmContainer.getContainerType() == ContainerType.BAG) {
                ordered = false;
            }
            if (cmContainer.getContainerType() == ContainerType.BAG || cmContainer.getContainerType() == ContainerType.SEQ) {
                unique = false;
            }
        }
        ...
        if (fieldType instanceof Class || fieldType instanceof Tuple || fieldType instanceof Container) {
            // Create EReference
    ...
    public void visit(Container container, Object param) {
        ...
        nrContainer++;
        eContainerClass.setName("ContainerClass_" + nrContainer);
        ...
        if (container.getType() instanceof Class || container.getType() instanceof Tuple || container.getType() instanceof Container) {
            // Create EReference
        ...
        if (container.getType() instanceof Container) {
            ...
            eContainerFeature.setOrdered(subType.getContainerType() == ContainerType.ORD || subType.getContainerType() == ContainerType.SEQ);
            eContainerFeature.setUnique(subType.getContainerType() == ContainerType.SET || subType.getContainerType() == ContainerType.ORD);
        } else {
    ...
    public void visit(Tuple tuple, Object param) {
        ...
        EPackage firstRoot = m_rootPackages.iterator().next();
        EClass eClass = g_EcoreFactory.createEClass();
        ...
        for (Type type : tuple.getTypes()) {
            Field typeField = new Field(Name.getName(getTupleElementName(tuple, typeIndex++)), type, 1, 1);
