...
/*
 * $Id: History.java,v 1.1.1.1 2007-03-20 10:05:18 kastenberg Exp $
 */
package groove.util;
...
/**
 * A history log that allows browsing back and forth, 
...
public class History<T> {
    ...
    /**
     * Indicates whether there exists a next element in the log.
     * This next element can then be retrieved using <tt>next()</tt>.
     * @return <tt>true</tt> if there exists a next element
     * @see #next()
     */
    public boolean hasNext() {
        if (DEBUG)
            Groove.message("Invoking History.hasNext at index "+index);
        return index < log.size() - 1;
    }

    /**
     * Indicates whether there exists a previous element in the log.
     * This previous element can then be retrieved using <tt>previous()</tt>.
     * @return <tt>true</tt> if there exists a previous element
     * @see #previous()
     */
    public boolean hasPrevious() {
        if (DEBUG)
            Groove.message("Invoking History.hasPrevious at index "+index);
        return index > 0;
    }

    /**
     * Returns the next element in the history log.
     * After invocation, the elements just returned will be the current one.
    ...
    public T next() {
        if (DEBUG)
            Groove.message("History.next() at index "+index);
        if (hasNext()) {
            index++;
    ...
    /**
     * Returns the current element in the history log.
     * Does not modify the log.
    ...
    public T current() {
        if (DEBUG)
            Groove.message("History.current() at index "+index);
        if (! isEmpty()) {
            return log.get(index);
        } else
    ...
    /**
     * Returns the previous element in the history log.
     * After invocation, the elements just returned will be the current one.
    ...
    /**
     * Replaces the current element in the history.
     * The new element will be the current and last one, i.e., all existing
     * next elements at the time of invocation will be discarded.
    ...
    public void replace(T element) {
        if (DEBUG)
            Groove.startMessage("History.replace("+element+") at index "+index);
        log.set(index, element);
        for (int i = log.size()-1; i > index; i--)
            log.remove(i);
        if (DEBUG)
            Groove.endMessage("History.replace("+element+") at index "+index);
    }
     ...
     * The new element will be the current and last one, i.e., all existing
     * next elements at the time of invocation will be discarded.
     * If the element to be added equals <tt>current()</tt>, nothing happens.
     * @param element the element to be added to the history
     * @ensure <tt>current() == element</tt>, <tt>! hasNext()</tt>
     * @see #replace(Object)
     */
    public void add(T element) {
        if (DEBUG)
            Groove.startMessage("History.add("+element+") at index "+index);
        if (index < 0 || ! element.equals(current())) {
        ...
        if (DEBUG)
            Groove.endMessage("History.add("+element+") at index "+index);
    }
