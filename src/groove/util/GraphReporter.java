 ...
 *
 * $Id: GraphReporter.java,v 1.1 2007-11-29 12:51:04 rensink Exp $
 */
package groove.util;
...
public class GraphReporter extends CommandLineTool {
     ...
     */
    public GraphReporter(List<String> args) {
        super(args);
    }
    ...
    private void report(Graph graph) {
        // count the labels
        ...
        for (Map.Entry<Label,? extends Bag.Multiplicity> labelEntry: labels.multiplicityMap().entrySet()) {
            println(String.format("%s\t%s", labelEntry.getKey(), labelEntry.getValue()));
        }
    }
