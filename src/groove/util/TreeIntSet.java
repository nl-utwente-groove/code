...
final public class TreeIntSet implements IntSet {
	...
	public void clear(int capacity) {
    	if (keys == null || keys.length <= capacity) {
    		keys = new int[2 * capacity];
    		store = new int[2 * width * capacity];
    	}
    	storeSize = 0;
    	size = 0;
	}
...
//    		} while (index > 0);
	...
	/**
	 * Reserves space for a new tree branch, and returns the index of 
	 * the first position of the new branch.
	 */
    private int newBranchIndex() {
    	int storeSize = this.storeSize;
    	int result = size == 0 ? 0 : storeSize;
    	storeSize += result == 0 ? ROOT_WIDTH : width;
    	if (storeSize >= store.length) {
			...
			Arrays.fill(store, result, storeSize, 0);
		}
