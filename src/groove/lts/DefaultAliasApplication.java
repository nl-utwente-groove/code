...
public class DefaultAliasApplication extends DefaultApplication implements
     ...
     * @param source the source graph of this application source state's
     *        predecessor, with the same event
     */
    public DefaultAliasApplication(RuleEvent event, GraphNextState source,
            GraphTransitionStub prior) {
    ...
    protected Node[] computeCreatedNodes() {
        Node[] result = this.prior.getAddedNodes(getParent());
        // if this application's event is the same as that of prior,
        // the added nodes may coincide
        if (result.length > 0
            && this.source.getEvent() == this.prior.getEvent(getParent())) {
            Node[] sourceAddedNodes = this.source.getAddedNodes();
    ...
    /**
     * Convenience method to retrieve the parent state of this application's
     * source.
     */
