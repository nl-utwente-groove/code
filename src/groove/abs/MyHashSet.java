...
/** A set of objects of type <T> with additional getAndAdd operation that makes atomic 
 * the test of containment followed by adding an element. This may be useful when
 * the test of equality for elements of T is costly. 
 * This implementation is based on a hash table and is parametrisable by an Equalizer
 * for testing equality (equivalence) of objects. Ensures constant time for basic 
 * operations (provided a good hash function). A Hasher for computing the hash code
...
public class MyHashSet<T> implements Iterable<T>, Set<T>{
	...
	/** Defines an equivalence relation over the objects of the same type
	 * and a compatible hash function. */ 
	...
	/** A combination of a get and an add operation. 
	 * This operation makes atomic a get (or containment operation) followed by an add operation
	 * in case the object is not yet in the set. When the element <code>o</code> is to be added,
	 * the cost of the operation is the same as for an add only (i.e. the test for containment is
	 * comes for free).
	 * @invariant getAndAdd(o) has the same effect as { e = get(o); if (e == null) { add(o); } return e; }
	 * @param o
	 * @return null if the object o was added to the set; the object equal to o contained in the set otherwise
	 */
	...
	/** Returns the object equal to <code>o</code> that is contained in this set, if any.
	 * @param o 
	 * @return the object equal to <code>o</code> contained in the set, if there is such an object; null otherwise 
	 */
	public T get (T o) {
	...
	public Iterator<T> iterator() {
		return new Iterator<T>() {
			/** The result of the following call of next() method, or null if there is no next element. 
			 * This field can be set by the hasNext() method when it tests for the existence of a next element. */
			private T next;
			...
			{ this.currentIdx = 0; this.currentBucketIt = MyHashSet.this.getElementsWithCode(0).listIterator(); this.next = null;} // Initialisation
			...
			public T next() {
				// if next has a value (i.e. this.next != null) -> return this.next
			...
			/** Goes to the next element to be returned by the following call to next() 
			 * @throws NoSuchElementException when no next element
			 */
			private void goToNext () throws NoSuchElementException {
				// if currentBucketIt has a next element, then advance to this next element and return
				// else, advance currendIdx and currentBucketIt to the next bucket and 
				// continue searching for the next element (recursive call)
				if (this.currentBucketIt.hasNext()) {
					this.next = this.currentBucketIt.next();
					return;
				}
				if (this.currentIdx == MyHashSet.this.store.capacity() - 1) { throw new NoSuchElementException(); }
				this.currentIdx ++; 
				this.currentBucketIt =MyHashSet.this.getElementsWithCode(this.currentIdx).listIterator();
				this.goToNext();
			}
		};
	}
	...
	/** The hash code for an object, depending whether a hasher is provided or not
	 * @param o
	...
	/** */
	protected Hasher<T> getHasher() { return this.hasher; }
	...
	private Hasher<T> getDefaultHasher () {
		return new Hasher<T> () {
			public boolean areEqual(Object o1, Object o2) { return o1.equals(o2); }
			public int getHashCode(Object o){ return o.hashCode(); }
		};
