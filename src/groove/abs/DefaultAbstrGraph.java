...
public class DefaultAbstrGraph extends DefaultGraph implements AbstrGraph {
	 ...
	 */
	public static DefaultAbstrGraph.Factory factory(PatternFamily family, int precision) {
	...
	public MultiplicityInformation multiplicityOf (Node n) {
		assert this.containsElement(n) : "Node " + n + " not is this shape";
	 ...
	 * @param p
	 * @return The unique node having type <code>p</code>, or null if such node does not exist.
	 */
	...
	public void removeFrom(Node n, int q) throws ExceptionRemovalImpossible {
		...
		this.type.put(n, new NodeType(c.getPattern(), Abstraction.MULTIPLICITY.remove(c.getMult(), q)));
		this.checkInvariants();
	}

	/** Ensures that the pattern is represented in the graph, and adds q to its multiplicity.
	...
	/** Two abstract graphs are isomorphic if there is an isomorphism on the underlying graph structure
		...
		public Morphism getIsomorphismToAbstrGraph (AbstrGraph other) {
			...
			for (VarNodeEdgeMap r : Util.getInjMatchesIter(this, other, map)) {
				...
				if (result.isSurjective()) { return result; }
			}
			this.checkInvariants();
			return null;
		}
	 ...
	 * A map into an abstract graph is injective if it respects multiplicities: 
	 * the number of pre-images of each nodes of the graph is less than its multiplicity.
	...
	public boolean isInjectiveMap (NodeEdgeMap om) {
		...
		Map<GraphPattern, Integer> maxAllowed = this.maxAllowed();
		for (Node n : map.nodeMap().values()) {
			Integer max = maxAllowed.get(this.typeOf(n));
	...
	/**
	 * Used to save a morphism used for the equals or quasiEquals functions. This is an optimisation in case the same equality test is performed twice one after the other.
	 */
	private static Morphism saveEqualsMorphism = null;

	public AbstrGraphsRelation compare (AbstrGraph other, boolean belongsIsSub) {		
		if (! this.family().equals(other.family()) || this.precision() != other.precision()) { return Abstraction.AbstrGraphsRelation.NOTEQUAL; }
		Morphism morphism;
		
		// test whether the same test was already performed recently
		if (saveEqualsMorphism != null && saveEqualsMorphism.dom() == this && saveEqualsMorphism.cod() == other) {
			morphism = saveEqualsMorphism;
		}
		else {
			morphism = this.getIsomorphismToAbstrGraph(other);
		}
		if (morphism == null) { return Abstraction.AbstrGraphsRelation.NOTEQUAL; }
		
		// if the morphism is not null, then the graphs are at least quasi-equal
		// the result won't be NOTEQUAL, thus save the morphism in case the next call compares the same graphs
		if (this.isFixed() && other.isFixed()) {
			saveEqualsMorphism = morphism;
		}
		
		Iterator<Map.Entry<Node,Node>> it = morphism.elementMap().nodeMap().entrySet().iterator();		
		...
		while (it.hasNext()  &&  result == AbstrGraphsRelation.EQUAL) {
			...
			MultInfoRelation compare = Abstraction.MULTIPLICITY.compare(this.multiplicityOf(entry.getKey()), other.multiplicityOf(entry.getValue()));
			switch (compare) {
			case M_EQUAL   : break;
			case M_SUBSET  : result = AbstrGraphsRelation.SUB; break;
			case M_SUPERSET: result = AbstrGraphsRelation.SUPER; break;
			case M_BELONGS : if (belongsIsSub) {result = AbstrGraphsRelation.SUB;} else {return AbstrGraphsRelation.QUASI;} break;
			case M_CONTAINS: if (belongsIsSub) {result = AbstrGraphsRelation.SUPER;} else {return AbstrGraphsRelation.QUASI;} break;
			case M_NOTEQUAL: return AbstrGraphsRelation.QUASI; 
			}
		}
		...
		while (it.hasNext()) {
			...
			MultInfoRelation compare = Abstraction.MULTIPLICITY.compare(this.multiplicityOf(entry.getKey()), other.multiplicityOf(entry.getValue()));
			switch (compare) {
			case M_EQUAL   : break;  // never happens
			case M_SUBSET: if (result != AbstrGraphsRelation.SUB) { return AbstrGraphsRelation.QUASI; } break;
			case M_SUPERSET: if (result != AbstrGraphsRelation.SUPER) { return AbstrGraphsRelation.QUASI; } break;
			case M_BELONGS : if (! belongsIsSub || result != AbstrGraphsRelation.SUB) { return AbstrGraphsRelation.QUASI; } break;
			case M_CONTAINS: if (! belongsIsSub || result != AbstrGraphsRelation.SUPER) { return AbstrGraphsRelation.QUASI; } break;
			case M_NOTEQUAL: return Abstraction.AbstrGraphsRelation.QUASI;
			}
		}
		return result;
	}
	 ...
	 */
	final public PatternFamily family() {	return this.family; }

	/**
	 * @return the precision of this shape
	 */
	final public int precision() { return this.precision; }
	...
	public boolean removeNode (Node n) {
		...
		GraphPattern typeOf = this.typeOf(n);
	...
	private Node ensureType (GraphPattern p){
		...
		if (n != null) { return n; }
		n = addNode();
		this.type.put(n, new NodeType(p, Abstraction.MULTIPLICITY.getElement(0, this.precision)));
		this.invType.put(p, n);
		this.checkInvariants();
		return n;
	}
	
	/** Computes the set of nodes that have a zero multiplicity with a given embedding.
	 * @param om The embedding.
	 * @return The set of nodes with zero multiplicity, or null if the embedding is not possible.
	 */
	 ...
	 * @param l
	 * @param targetPattern
	 * @require sourcePattern and targetPattern are present in the abstract graph
	 * @return The edge possibly already present between these nodes, or null if there was no such edge.
	 */
	public Edge addEdgeBetweenPatterns(GraphPattern sourcePattern, Label l, GraphPattern targetPattern) {
		return addEdge(this.nodeFor(sourcePattern), l, this.nodeFor(targetPattern));
	}
	
	/** Adds an edge between the node corresponding to some pattern and another node.
	 * @param sourcePattern
	 * @param l
	 * @param targetNode
	 ...
	 * @return The edge possibly already present between these nodes, or null if there was no such edge.
	 */
	public Edge addEdgeBetweenPatterns(GraphPattern sourcePattern, Label l, Node targetNode) {
		return addEdge(this.nodeFor(sourcePattern), l, targetNode);
	}
	 ...
	 * @return The edge possibly already present between these nodes, or null if there was no such edge.
	 */
	public Edge addEdgeBetweenPatterns(Node sourceNode, Label l, GraphPattern targetPattern) {
		return addEdge(sourceNode, l, this.nodeFor(targetPattern));
	}
	...
	public class Factory {
		 ...
		 */
		final public PatternFamily getFamily() { return this.myFamily; }
	
		/**
		 * @return the precision of the factory
		 */
		final public int getPrecision() { return this.myPrecision; }
		...
		public DefaultAbstrGraph getShapeGraphFor (Graph graph) throws ExceptionIncompatibleWithMaxIncidence {
			...
			for (Node currNode : graph.nodeSet()) {
				GraphPattern pattern = this.myFamily.computeAddPattern(graph, currNode);
	...
	/**
	 * This implemention uses the criterion that, for a graph with concretisations,
	 * the type of all node should be present in its neighbourhood.
	 * That is, returns false if for some <code>node</code> in the graph,
	 * there is no morphism from the type of <code>node</code> into the graph
	 * that maps the center of the type with <code>node</code>.
	 */
	public boolean isWithoutConcretisation () {
		for (Map.Entry<GraphPattern, Node> entry : this.invType.entrySet()) {
			...
			for (VarNodeEdgeMap match: Util.getMatchesIter(pattern, this, map)) {
				if (this.isInjectiveMap(match)) { 
					hasInjective = true;
					break;
				}
			}
			if (! hasInjective) { return true; }
		}
		return false;
	}
