	...
	public Set<? extends GraphTransition> applyMatch(GraphState source, RuleMatch match, ExploreCache cache) {
		...
		for (AbstrGraph transf : transfResult) {
			...
			if (transf != DefaultAbstrGraph.INVALID_AG) {
				AbstrGraphNextState newState = new AbstrGraphNextStateImpl(transf, abstrSource, event);
				AbstrGraphState oldState = (AbstrGraphState) this.getGTS().addState(newState);
				if (oldState != null) {
					// the state was not added as an equivalent state existed
					trans = new AbstrGraphTransitionImpl(abstrSource, event, oldState);
				} else {
					// the state was added as a next-state
					trans = newState;
				}
			} else {
				trans = new AbstrGraphTransitionImpl(abstrSource, event, AGTS.INVALID_STATE);
			}
