 ...
 * the License.
 * 
 * $Id: DeltaGraph.java,v 1.13 2008-01-30 09:32:57 iovka Exp $
 */
...
/**
 * A delta graph is a graph that may store only the <i>changed elements</i>
 * with respect to another, <i>basis</i> graph. This brings a huge potential
 * reduction in storage, at the penalty of computation time, since the graph
 * elements may have to be reconstructed from the basis modulo the changes. This
 * implementation caches the element set so as to avoid too frequent
 * reconstruction.
...
public class DeltaGraph extends AbstractGraph<DeltaGraphCache> implements
    ...
    public DeltaGraph(AbstractGraph<? extends GraphCache> graph) {
        assert graph.isFixed() : "Don't create delta graph on top of unfixed graph "
            + graph;
        this.basis = graph;
        deltaGraphCount++;
    }
    ...
    /**
     * Sets the delta using {@link #computeFixedDeltaArray()}. In either case
     * the super implementation is called at the end.
     */
    @Override
    ...
    /**
     * If this graph is fixed, this implementation returns a {@link DeltaGraph}
     * with this one as basis; otherwise, it returns a {@link DefaultGraph}.
     */
    @Override
    public Graph clone() {
        Graph result =
    ...
    /**
     * Clears the cache of this graph, using {@link #clearCache()}, and then
     * recursively clears the cache of the basis, if any. Used for accounting
     * purposes.
     */
    public void clearAllCaches() {
        clearCache();
        if (this.basis instanceof DeltaGraph) {
            ((DeltaGraph) this.basis).clearAllCaches();
        } else {
            this.basis.clearCache();
        }
    }

    /**
     * Returns the basis of this delta graph. May be <tt>null</tt> if the
     * graph has no basis.
     */
    ...
    /**
     * Indicates if this graph is <i>frozen</i> rather than a proper delta
     * graph. The decision to freeze the graph is taken at the time it is fixed.
     * @see #setFrozen()
     */
    ...
    /**
     * Fixes the graph by freezing it. This means that the basis is set to
     * <tt>null</tt> and all graph elements are considered to have been added.
     * Furthermore, the basis is reversed, i.e., the newly frozen graph is
     * offered to the current basis as its new basis. The graph should be fixed
     * before invoking this method. Callback method from {@link #setFixed()}.
    ...
    /**
     * Resets the basis of this delta graph, and suggests to the old basis that
     * it take this graph as its basis instead.
     */
    protected void invertBasis() {
        Graph oldBasis = this.basis;
    ...
    /**
     * Changes the basis of this delta graph to another. Also offers this graph
     * as a new basis to the previous basis, if this improves the delta for the
     * previous basis. TODO: Currently disabled
    ...
    protected synchronized void invertBasis(DeltaGraph newBasis,
        ...
        if (false) {// newBasis.getDepth() + 1 < getDepth()) {
            ...
            // compute the number of elements in the delta array that are
            // actually not delta elements
            int restSize = oldDelta.length - oldDeltaSize;
            // compute the new delta by reversing the added and removed
            // sub-arrays in the parameters
            this.basis = newBasis;
            ...
            if (reverseDeltaSize == 0) {
                if (restSize == 0) {
                    this.deltaArray = EMPTY_ELEMENT_ARRAY;
                } else {
                    this.deltaArray = new Element[restSize];
                }
            } else if (reverseAddedLength == 0) {
                // there are no added elements in reverse, so no removed
                // elements in this
                this.deltaArray = new Element[reverseDeltaSize - 1 + restSize];
                System.arraycopy(invertedDelta, 1, this.deltaArray, 0,
                    reverseDeltaSize - 1);
            } else if (reverseAddedLength == reverseDeltaSize) {
                // there are no removed elements in reverse, so no added
                // elements in this
                this.deltaArray = new Element[reverseDeltaSize + 1 + restSize];
                System.arraycopy(invertedDelta, 0, this.deltaArray, 1,
                    reverseDeltaSize);
            } else {
                this.deltaArray = new Element[reverseDeltaSize + restSize];
                int addedLength = reverseDeltaSize - reverseAddedLength - 1;
                System.arraycopy(invertedDelta, 0, this.deltaArray,
                    addedLength + 1, reverseAddedLength);
                System.arraycopy(invertedDelta, reverseAddedLength + 1,
                    this.deltaArray, 0, addedLength);
            }
            // copy the non-delta elements from the old to the new delta
            System.arraycopy(oldDelta, oldDeltaSize, this.deltaArray,
                this.deltaArray.length - restSize, restSize);
            clearCache();
    ...
    /**
     * Returns the length of the delta chain for this graph. The delta chain is
     * the chain of graphs composed from each other by deltas upon a basis
     * graph.
     */
    protected int getDepth() {
        if (isFrozen()) {
            ...
            int basisDepth =
                this.basis instanceof DeltaGraph
                        ? ((DeltaGraph) this.basis).getDepth() : 0;
            return basisDepth + 1;
        }
    }

    /**
     * This implementation returns a {@link DeltaGraphCache}. Note that the
     * cache will attempt to initialise itself using the basis' node and edge
     * sets, if this graph is not fixed.
     */
    @Override
    ...
    /**
     * Returns the delta array, or <tt>null</tt> as long as the delta is not
     * set.
     */
    ...
    protected boolean isDeltaArraySet() {
        return this.deltaArray != null;
    }

    /**
     * Returns the number of {@link Element} entries in {@link #getDeltaArray()}.
     * Normally this would equals <code>delta.length</code>, but if the delta
     * array is used for other things as well, then it might be less.
     */
    protected int getDeltaSize() {
        return this.deltaArray.length;
    }
    ...
    /**
     * Computes the array of added and removed element sets on the basis of the
     * current graph cache.
     */
    ...
    /**
     * Computes the array of added and removed element sets on the basis of a
     * given delta store.
     */
    ...
    /**
     * Delivers an array containing all elements in this graph, so that the
     * graph can be reconstructed without the basis.
     */
    ...
    /**
     * Stores the frozen delta, consisting of all the nodes and edges of the
     * graph, into a given element array, from a given position. The array has
     * to be initialized to the correct length before invocation.
     * @param result the array into which the frozen delta is to be stored
     * @param prefixSize the index of the first element to store the frozen
     *        delta
     */
     ...
     * THat is, <code>action.added(elem)</code> is called for each element
     * <code>elem</code> added in the delta, and
     * <code>action.removed(elem)</code> for each removed element.
     * @param target the action object to be called back
     * @param mode indication if callback should only be done only for edges ({@link DeltaApplier#EDGES_ONLY}),
     *        only for nodes ({@link DeltaApplier#NODES_ONLY}) or for all
     *        elements ({@link DeltaApplier#ALL_ELEMENTS})
     */
