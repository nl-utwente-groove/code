...
class VariableNodeSearchItem extends AbstractSearchItem {
     ...
     */
    public VariableNodeSearchItem(VariableNode node) {
        this.node = node;
        this.boundNodes = Collections.<Node>singleton(node);
    }

    public VariableNodeRecord getRecord(SearchPlanStrategy.Search matcher) {
        return new VariableNodeRecord(matcher);
    }
...
//    final ValueNode nodeImage;
     ...
     * @version $Revision $
     */
    private class VariableNodeRecord extends SingularRecord {
        /**
         * Creates a record based on a given underlying matcher.
         */
        VariableNodeRecord(Search search) {
            super(search);
        }
        ...
        @Override
        public void reset() {
            this.search.putNode(VariableNodeSearchItem.this.nodeIx, null);
        }

        @Override
        public String toString() {
            return VariableNodeSearchItem.this.toString();
        }
    }
}
