...
public class ExplorationDialog extends JDialog implements ActionListener {
    ...
    private static final String     FULL_EXPLORATION        = "Full exploration";           // strategy type
    private static final String     PATH_EXPLORATION        = "Path exploration";           // strategy type
    private static final String     BRANCHING               = "Branching";                  // full exploration strategy
    private static final String     BREADTH_FIRST           = "Breadth First";              // full exploration strategy
    private static final String     DEPTH_FIRST             = "Depth First";                // full exploration strategy
    private static final String     LINEAR_CONFLUENCE       = "Linear Confluence";          // full exploration strategy
    private static final String     LINEAR                  = "Linear";                     // path exploration strategy
    private static final String     RANDOM_LINEAR           = "Random Linear";              // path exploration strategy
    private static final String     NONE_ACCEPTOR           = "None";                       // acceptor
    private static final String     FINAL_STATE             = "Final State";                // acceptor
    private static final String     RULE_MATCH              = "Rule Match";                 // acceptor (with rule)
    private static final String     RULE_MISMATCH           = "Rule Mismatch";              // acceptor (with rule)
    private static final String     EXPLORE_COMMAND         = "Explore State Space";        // button command
    ...
    // * isConditionalAcceptor       - boolean to indicate if the acceptor is conditional (needs rule) or not
    ...
    // path exploration. This panel is referenced to change the visibility of its CardLayout.
    private JPanel eitherStrategySelection;
    ...
    private JPanel optionalRulePanel;
    ...
    public ExplorationDialog(Simulator simulator, JFrame owner) {
        ...
        methodSelection.addOption(ExplorationDialog.FULL_EXPLORATION,            "explore all rule matches in each reached state");
        methodSelection.addOption(ExplorationDialog.PATH_EXPLORATION,            "explore exactly one rule match in each reached state");
        ...
        FormattedSelection pathStrategySelection = new FormattedSelection("Choose a strategy for the path exploration:", this);
        ...
        this.eitherStrategySelection.add(fullStrategySelection, ExplorationDialog.FULL_EXPLORATION);
        this.eitherStrategySelection.add(pathStrategySelection, ExplorationDialog.PATH_EXPLORATION);
        ...
        acceptorSelection.addOption(ExplorationDialog.NONE_ACCEPTOR,             "continue until no further matches are available");
        acceptorSelection.addOption(ExplorationDialog.FINAL_STATE,               "stop as soon as a final state is encountered");
        acceptorSelection.addOption(ExplorationDialog.RULE_MATCH,                "stop as soon as the selected rule matches");
        acceptorSelection.addOption(ExplorationDialog.RULE_MISMATCH,             "stop as soon as the selected rule does not match");
        dialogContent.add(new JLabel(" "));
        dialogContent.add(acceptorSelection);
        
        // Query for the transformation rule (in case of RULE_MATCH or RULE_MISMATCH)
    ...
    // The action listener of the dialog. Uses the String content of the message to
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        {
            this.isFullExploration = (event.getActionCommand() == ExplorationDialog.FULL_EXPLORATION);
            CardLayout cards = (CardLayout) (this.eitherStrategySelection.getLayout());
            ...
            event.getActionCommand() == ExplorationDialog.FINAL_STATE)
        {
            ...
            CardLayout cards = (CardLayout) (this.optionalRulePanel.getLayout());
    ...
    // Return the acceptor associated with the current selection state of the dialog.
    // Should only be called when 'isConditionalAcceptor' is false.
    private Acceptor getAcceptor() {
        if (this.selectedAcceptor == ExplorationDialog.NONE_ACCEPTOR)               return (new Acceptor());
        if (this.selectedAcceptor == ExplorationDialog.FINAL_STATE)                 return (new FinalStateAcceptor(new Result(1)));
    
        // Default case. Should never be reached.
        return (new Acceptor());
    }

    // Return the scenario associated with the current selection state of the dialog.
    private Scenario getScenario() {
        if (!this.isConditionalAcceptor)
            return ScenarioFactory.getScenario(getStrategy(), getAcceptor(), "", "");
        
        AspectualRuleView rule = this.simulator.getCurrentGrammar().getRuleMap().get(this.selectedRule);
        return ScenarioFactory.getConditionalScenario(
            getStrategy(),
    ...
    // Return the strategy associated with the current selection state of the dialog.
    private Strategy getStrategy() {
        if (this.isFullExploration) {
            if (this.selectedFullStrategy == ExplorationDialog.BRANCHING)           return (new BranchingStrategy());
            if (this.selectedFullStrategy == ExplorationDialog.DEPTH_FIRST)         return (new ExploreRuleDFStrategy());
            if (this.selectedFullStrategy == ExplorationDialog.BREADTH_FIRST)       return (new BFSStrategy());
            if (this.selectedFullStrategy == ExplorationDialog.LINEAR_CONFLUENCE)   return (new LinearConfluentRules());
        } else {
            if (this.selectedPathStrategy == ExplorationDialog.LINEAR)              return (new LinearStrategy());
            if (this.selectedPathStrategy == ExplorationDialog.RANDOM_LINEAR)       return (new RandomLinearStrategy());
        }
    ...
    private class FormattedSelection extends JPanel {
        ...
        // Local copy of the ActionListener (which is the surrounding Dialog as a whole)
        private final ActionListener listener;
        
        // Variable used to identify the first check box (which should be selected initially).
