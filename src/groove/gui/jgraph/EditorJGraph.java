 ...
 *
 * $Id: EditorJGraph.java,v 1.2 2007-03-28 15:12:27 rensink Exp $
 */
...
/**
 * A j-graph dedicated towards the editor.
...
public class EditorJGraph extends JGraph {           
     ...
     * @param editor the {@link IEditorModes} to which this j-graph is associated
     * @since june2005 
     */
    public EditorJGraph(IEditorModes editor) {
        super(new EditorJModel());
        this.editor = editor;
    ...
    public EditorJModel getModel() {
    	return (EditorJModel) graphModel;
    }

    /**
     * @return the editor{@link IEditorModes} to which this j-graph is associated
     */
    ...
    /** 
     * Adds a j-vertex to the j-graph, and positions it at a given point. 
     * The point is in screen coorinates
    ...
    protected void addVertex(Point2D screenPoint) {
        ...
        Point2D corner = new Double(atPoint.getX() - size.width / 2, atPoint.getY()
                - size.height / 2);
        GraphConstants.setBounds(jVertex.getAttributes(), new SerializableRectangle2D(corner.getX(),
                corner.getY(), size.getWidth(), size.getHeight()));
        ...
        if (startEditingNewNode) {
            startEditingAtCell(jVertex);
        }
    }

    /**
     * Adds an edge beteen two given points.
     * The edge actually goes from the vertices underlying the points.
     * The end point may not be at a vertex, in which case a self-edge should be drawn.
     * The points are given in screen coordinates.
    ...
    protected void addEdge(Point2D screenFrom, Point2D screenTo) {
        ...
        if (startEditingNewEdge) {
            startEditingAtCell(newEdge);
        }
    }

    /**
     * Callback method to determine whether an event concerns edge creation. To be overridden by
     * subclasses.
    ...
    /**
     * Callback method to determine whether an event concerns node creation. To be overridden by
     * subclasses.
    ...
    protected boolean isNodeMode(MouseEvent evt) {
        ...
        return (getEditor().isNodeMode()
                && getFirstCellForLocation(evt.getX(), evt.getY()) == null);
    }

    /**
     * Flag to indicate creating a node will immediately start aditing the node label
     */
    protected boolean startEditingNewNode = false;
    /**
     * Flag to indicate creating an edge will immediately start aditing the edge label
     */
