...
//        
// //Iterator<Set<Rule>> ruleSetIter =
// record.getRuleSystem().getRuleMap().values().iterator();
...
// // if i'm dealing with lambdas i have to use getApplications, cant be done
// lazy
//        	
//        	
// // this would implement the stuff considering an iterator over all
// applications
// // but it doesn't matter yet for linear exploration since it would only do
// the
...
// }
//        	
// // make sure it continues when the current result yields only lambda
// transitions
// // except for else transitions
// }
// reporter.stop();
// return result == null ? Collections.<RuleApplication>emptySet().iterator() :
// result;
// }
//	
// /**
...
// Iterator<RuleApplication> result = new
// AbstractNestedIterator<RuleApplication>() {
// @Override
// protected boolean hasNextIterator() {
// while (!atEnd && nextIter == null && ruleIter.hasNext()) {
// final Rule nextRule = ruleIter.next();
// nextIter = new TransformIterator<RuleMatch,
// RuleApplication>(nextRule.getMatchIter(getGraph(), null)) {
// @Override
...
// Set<Rule> rules = null;
//
// // the commented lines would be a fix for collecting all possible
// applications
// // when we have lambda transitions also. not needed for full or linear (where
// in
...
// */
// protected void collectApplications(Set<Rule> rules, Set<RuleApplication>
// result) {
...
//	
//	
// protected boolean doApplications(Action action, Iterator<Set<Rule>> iter) {
// boolean done = false;
...
// record.getRuleSystem().getRuleMap().values().iterator();
// //Iterator<Set<Rule>> ruleSetIter = getRuleSetIter();
// done = doApplications(action, getRuleSetIter());
// return done;
// }
//
// /**
// * Attempts to apply a given set of rules to the graph, and performs a given
// action
