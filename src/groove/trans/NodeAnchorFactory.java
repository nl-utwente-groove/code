 ...
 * expressions, and the ability to generate a minimal representation of the
 * morphism in the form of an array of node images, as well as the ability to
 * reconstruct itself out of such a minimal representation.
...
public class NodeAnchorFactory implements AnchorFactory<Rule> {
    ...
    /**
     * A fixed matching prototype, to be used as return value in
     * <tt>getPrototype()</tt>.
     */
    static private NodeAnchorFactory prototype = new NodeAnchorFactory();

    /**
     * This implementation returns an array consisting of the nodes in the
     * rule's left hand side.
     */
    public Element[] newAnchors(Rule rule) {
        Set<Element> anchors = new HashSet<Element>(rule.lhs().nodeSet());
        // set of edge ends that may be removed because the edges themselves are
        // anchors
