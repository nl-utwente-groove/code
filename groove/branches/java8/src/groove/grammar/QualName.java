...
public class QualName implements Comparable<QualName> {
     ...
     */
    public @NonNull String child() {
        String result = get(size() - 1);
        assert result != null;
        return result;
    }
    ...
    public static @NonNull String lastName(String fullName) {
        try {
     ...
     */
    private static boolean isValidStarter(char character, StringBuilder legal, StringBuilder error) {
    ...
    public static boolean isValid(String id, StringBuilder legal, StringBuilder error) {
        ...
        for (int i = 0; i < id.length(); i++) {
            if (id.charAt(i) == SEPARATOR_CHAR) {
                valid = valid && !first;
                if (first && error != null && error.length() == 0) {
                    if (i == 0) {
                ...
                if (first) {
                    valid = isValidStarter(id.charAt(i), legal, error) && valid;
                } else {
                    valid =
