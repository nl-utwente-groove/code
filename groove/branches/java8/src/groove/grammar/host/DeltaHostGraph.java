...
public final class DeltaHostGraph extends AGraph<HostNode,HostEdge> implements HostGraph, Cloneable {
    ...
    @Override
    public HostEdge addEdge(@NonNull HostNode source, @NonNull Label label, @NonNull HostNode target) {
        throw new UnsupportedOperationException();
    }
     ...
     */
    @Override
    public boolean removeEdge(@NonNull HostEdge edge) {
        throw new UnsupportedOperationException();
    }
     ...
     */
    @Override
    public boolean addEdge(@NonNull HostEdge edge) {
        throw new UnsupportedOperationException();
    }
     ...
     */
    @Override
    public boolean removeNode(@NonNull HostNode node) {
        throw new UnsupportedOperationException();
    }

    @Override
    public Set<@NonNull HostNode> nodeSet() {
        if (this.nodeEdgeStore == null) {
            initData();
        }
        Set<@NonNull HostNode> result = this.nodeEdgeStore.keySet();
    ...
    @Override
    public HostEdgeSet inEdgeSet(@NonNull Node node) {
    ...
    @Override
    public HostEdgeSet outEdgeSet(@NonNull Node node) {
        HostEdgeSet result = getOutEdgeStore().get(node);
        return (ALIAS_SETS || this.copyData) && result != null ? result : createEdgeSet(result);
    }

    /** Returns a mapping from nodes to sets of outgoing edges. */
    private HostEdgeStore<@NonNull HostNode> getOutEdgeStore() {
     ...
     * method is only used if the map could not be obtained from the basis.
     */
    private HostEdgeStore<@NonNull HostNode> computeOutEdgeStore() {
        HostEdgeStore<@NonNull HostNode> result = new HostEdgeStore<>();
        for (Map.Entry<HostNode,HostEdgeSet> nodeEdgeEntry : this.nodeEdgeStore.entrySet()) {
            HostNode key = nodeEdgeEntry.getKey();
            HostEdgeSet inEdges = createEdgeSet(null);
            for (HostEdge edge : nodeEdgeEntry.getValue()) {
    ...
    @Override
    public HostEdgeSet edgeSet(@NonNull Label label) {
    ...
    @Override
    public HostEdgeSet edgeSet(@NonNull Node node) {
    ...
    private void initData() {
        if (!isDataInitialised()) {
            ...
            if (this.basis == null) {
                ...
                this.nodeEdgeStore = new HostEdgeStore<@NonNull HostNode>();
     ...
     */
    HostEdgeSet createEdgeSet(Set<@NonNull HostEdge> edgeSet) {
        return HostEdgeSet.newInstance(edgeSet);
    }

    HostNodeSet createNodeSet(Set<@NonNull HostNode> nodeSet) {
    ...
    HostEdgeStore<@NonNull HostNode> nodeEdgeStore;
    /** The map from nodes to sets of incoming edges. */
    HostEdgeStore<@NonNull HostNode> nodeInEdgeStore;
    /** The map from nodes to sets of outgoing edges. */
    HostEdgeStore<@NonNull HostNode> nodeOutEdgeStore;
    ...
    abstract private class DataTarget implements DeltaTarget {
         ...
         */
        final boolean addEdge(@NonNull HostEdge edge, boolean refreshSource, boolean refreshTarget,
