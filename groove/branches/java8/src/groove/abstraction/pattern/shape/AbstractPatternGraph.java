...
public abstract class AbstractPatternGraph<N extends AbstractPatternNode,E extends AbstractPatternEdge<N>>
    ...
    protected final List<Set<@NonNull N>> layers;
    ...
    protected AbstractPatternGraph(String name) {
        super(name);
        this.depth = 0;
        this.layers = new ArrayList<Set<@NonNull N>>();
    }
    ...
    @Override
    public Set<@NonNull N> nodeSet() {
        return (Set<@NonNull N>) super.nodeSet();
    }

    @SuppressWarnings("unchecked")
    @Override
    public Set<@NonNull E> edgeSet() {
        return (Set<@NonNull E>) super.edgeSet();
    }

    @SuppressWarnings("unchecked")
    @Override
    public Set<@NonNull E> edgeSet(@NonNull Node node) {
        return (Set<@NonNull E>) super.edgeSet(node);
    }

    @SuppressWarnings("unchecked")
    @Override
    public Set<@NonNull E> outEdgeSet(@NonNull Node node) {
        return (Set<@NonNull E>) super.outEdgeSet(node);
    }

    @SuppressWarnings("unchecked")
    @Override
    public Set<@NonNull E> inEdgeSet(@NonNull Node node) {
        return (Set<@NonNull E>) super.inEdgeSet(node);
    }
    ...
    public boolean isWellFormed() {
        ...
        for (N pNode : getLayerNodes(0)) {
            ...
            if (pattern.nodeCount() != 1 || Util.getBinaryEdgesCount(pattern) != 0) {
                return false;
            }
        }
    ...
    public Set<@NonNull N> getLayerNodes(int layer) {
        assert layer >= 0;
        ...
        for (E pEdge : inEdgeSet(pNode)) {
            ...
            if (sEdges != null) {
                for (HostEdge sEdge : sEdges) {
                    if (pEdge.isCod(sEdge.source()) && pEdge.isCod(sEdge.target())) {
                        edges.remove(sEdge);
                    }
                }
            }
        ...
        while (!queue.isEmpty()) {
            ...
            for (E pEdge : coveringEdges) {
                ...
                if (newPN.getLayer() == 0) {
                    ...
                    Pair<N,HostNode> newPair = new Pair<N,HostNode>(newPN, newSN);
        ...
        while (!queue.isEmpty()) {
            ...
            for (E pEdge : coveringEdges) {
                ...
                if (newPN.getLayer() == 1 && newPN.introduces(newSE)) {
                    ...
                    Pair<N,HostEdge> newPair = new Pair<N,HostEdge>(newPN, newSE);
                    if (!queue.contains(newPair)) {
                        queue.add(newPair);
                    }
                }
     ...
     */
    public SortedSet<N> getDownwardTraversal(List<@NonNull N> toTraverse) {
    ...
    public Duo<E> getIncomingEdges(@NonNull N node) {
