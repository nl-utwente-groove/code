...
public final class PatternRule {
    ...
    private final @Nullable Rule sRule;
    ...
    private @Nullable RuleNode[] eraserNodes;
    private @Nullable RuleEdge[] eraserEdges;
    private @Nullable RuleNode[] creatorNodes;
    private @Nullable RuleEdge[] creatorEdges;
    ...
    public RuleNode addReaderNode(@NonNull TypeNode tNode) {
        RuleNode result = createNode(tNode);
        addToLhs(result);
        addToRhs(result);
        return result;
    ...
    private void addToLhs(@NonNull RuleNode rNode) {
        this.lhs.addNode(rNode);
    }

    private void addToLhs(@NonNull RuleEdge rEdge) {
        this.lhs.addEdgeContext(rEdge);
    }

    private void addToRhs(@NonNull RuleNode rNode) {
        this.rhs.addNode(rNode);
    }

    private void addToRhs(@NonNull RuleEdge rEdge) {
    ...
    private @NonNull RuleEdge createEdge(RuleNode rSrc, TypeEdge tEdge, RuleNode rTgt) {
    ...
    private RuleNode @NonNull [] computeEraserNodes() {
        @NonNull
        Set<RuleNode> result = new MyHashSet<RuleNode>();
        result.addAll(lhs().nodeSet());
        result.removeAll(rhs().nodeSet());
        return result.toArray(new RuleNode @NonNull [result.size()]);
    }

    private @NonNull RuleEdge @NonNull [] computeEraserEdges() {
    ...
    private @NonNull RuleNode @NonNull [] computeCreatorNodes() {
        Set<RuleNode> result = new MyHashSet<RuleNode>();
    ...
    private @NonNull RuleEdge @NonNull [] computeCreatorEdges() {
        Set<RuleEdge> result = new MyHashSet<RuleEdge>();
    ...
    public void fixCommutativity() {
        ...
        RuleNode creatorNode = rhs().getLayerNodes(rhs().depth()).iterator().next();
        Map<RuleNode,RuleNode> replacementMap = new MyHashMap<RuleNode,RuleNode>();
