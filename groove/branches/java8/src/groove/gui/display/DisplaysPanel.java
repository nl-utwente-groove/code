...
public class DisplaysPanel extends JTabbedPane implements SimulatorListener {
    ...
    private boolean showOrHideTab(ResourceKind resource) {
        ...
        if (show) {
            ...
            remove(display);
            Optional<ListPanel> listPanel = display.getListPanel();
            if (listPanel.isPresent()) {
                Optional<JTabbedPane> listsPanel = getListsPanel(displayKind);
                assert listsPanel.isPresent() : "Implied by presence of listPanel";
                listsPanel.get().remove(listPanel.get());
            }
        }
        return show;
    }
     ...
     */
    public @NonNull JTabbedPane getUpperListsPanel() {
     ...
     * @see Display#getListPanel()
     */
    public @NonNull JTabbedPane getLowerListsPanel() {
    ...
    /**
     * Returns the upper or lower lists panel, depending on the display kind.
     * @see DisplayKind#getListPanel()
     */
    private Optional<JTabbedPane> getListsPanel(DisplayKind kind) {
        Optional<JTabbedPane> result;
        switch (kind.getListPanel()) {
        case UPPER:
            result = Optional.of(getUpperListsPanel());
            break;
        case LOWER:
            result = Optional.of(getLowerListsPanel());
            break;
        case NONE:
    ...
    public void attach(Display display) {
        ...
        // first add the corresponding list panel, if any
        Optional<? extends JPanel> listPanel = display.getListPanel();
        if (listPanel.isPresent()) {
            JTabbedPane listsPanel = getListsPanel(display.getKind()).get();
            assert listsPanel != null : "Guaranteed by presence of listPanel";
            if (listsPanel.indexOfComponent(listPanel.get()) < 0) {
                int index;
