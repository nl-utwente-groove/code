...
/*
 * $Id: Colors.java,v 1.1.1.1 2007-03-20 10:05:17 kastenberg Exp $
 */
package groove.util;
...
public class Colors {
    ...
    /**
     * Returns a map from color names to colors.
     * The keys include all field names from <tt>java.awt.Color</tt> as strings,
     * plus keys <tt>key.darker</tt> and <tt>key.brighter</tt> for each of 
     * <tt>key</tt>. 
     * @return An (unmodifiable) map from <tt>String</tt> values to <tt>Color</tt>
     * instances.
     */
    ...
    /**
     * Searches for a color with a given name or key, given as a string. The color is
     ...
     * then the corresponding color is returned using the appropriate <tt>Color</tt>
     * method;
     * <li> Otherwise, the color cannot be found and <tt>null</tt> is returned. 
     * </ul>  
     * @param name the name or key under which the color is sought
     * @return the color found for <tt>name</tt>, or <tt>null</tt> if no such color
     * is found.
     * @see #getColorMap()
...
//                int alpha = Integer.parseInt(values[3]);
    ...
    /**
     * Prints out the (key, color)-pairs in the map returned by <tt>getColorMap()</tt>,
     * in the form <tt>key = hexString</tt> (one per line).
     * @see #getColorMap()
     */
     ...
     */
    private static final Map<String,Color> colorMap = new TreeMap<String,Color>();
