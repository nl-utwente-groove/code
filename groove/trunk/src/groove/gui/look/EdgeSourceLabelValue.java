...
/**
 * Value of the edge source label (typically the outgoing multiplicity).
 * @author Arend Rensink
 * @version $Revision $
 */
public class EdgeSourceLabelValue extends AspectValue<String> {
    @Override
    ...
    protected String getForJEdge(AspectJEdge jEdge) {
        ...
        if (edge != null && edge.getGraphRole() == GraphRole.TYPE) {
            Multiplicity outMult = edge.getOutMult();
            if (outMult != null) {
                result = outMult.toString();
            }
        }
        return result;
    }
