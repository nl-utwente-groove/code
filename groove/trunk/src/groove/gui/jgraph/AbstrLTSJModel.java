...
public class AbstrLTSJModel extends LTSJModel {
	...
	AbstrLTSJModel(LTS lts, Options options) {
		...
		assert lts instanceof AGTS : "Wrong type : " + lts + " should be of type AGTS.";
	}
    ...
    protected AttributeMap createJEdgeAttr(Set<? extends Edge> edgeSet) {
        ...
        if (this.activeTransitionsSet != null && this.activeTransitionsSet.containsAll(edgeSet)) {
...
//        }
    ...
    public void setActive(State state, Transition trans) {
		...
		if (previousActiveTransitions == null || ! previousActiveTransitions.contains(trans)) {
			if (trans == null) {
				this.activeTransitionsSet = null; 
			} else {
				this.activeTransitionsSet = ((AGTS) this.getGraph()).getEquivalentTransitions((AbstrGraphTransition) trans);
				for (Transition t: this.activeTransitionsSet) {
					JCell jCell = getJCell(t);
					assert jCell != null : String.format("No image for %s in jModel", t);
	...
	/** Replaces the activeTransition field, as a set of transitions may be active
	 * All active transitions have the same source and the same rule event component
	 */
	Collection<Transition> activeTransitionsSet;
	
	@Override
	/** This implementation returns a random transition between the active transitions, if any. */
	public Transition getActiveTransition() {
		if (this.activeTransitionsSet != null && ! this.activeTransitionsSet.isEmpty()) {
	...
	static public final AbstrLTSJModel EMPTY_ABSTR_LTS_JMODEL = new AbstrLTSJModel();
}
