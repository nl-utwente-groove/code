 ...
 * the License.
 * 
 * $Id: GenericNodeEdgeMap.java,v 1.2 2008-01-30 09:32:50 iovka Exp $
 */
...
/**
 * Interface combining the functionality of two maps, one for nodes and the
 * other for edges. The map is completely generic, meaning that it can be
 * instantiated with any source and target types that are claimed to be node and
 * edge types. Consistency between the node and edge maps is not checked.
 * <ul>
 * <li><code>NS</code> is the <i>node source type</i>, i.e., the type of
 * node keys
 * <li><code>NT</code> is the <i>node source type</i>, i.e., the type of
 * node images
 * <li><code>ES</code> is the <i>edge target type</i>, i.e., the type of
 * edge keys
 * <li><code>ET</code> is the <i>edge target type</i>, i.e., the type of
 * edge images
 * </ul>
