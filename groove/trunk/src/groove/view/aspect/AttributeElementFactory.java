...
public class AttributeElementFactory {
     ...
     */
    public VariableNode createValueNode(AspectNode node) {
        VariableNode result;
        AspectValue dataType = node.getType();
        ...
        if (constant == null) {
            result = VariableNode.createVariableNode(node.getNumber(), algebra);
        } else {
            result =
                ValueNode.createValueNode(algebra, algebra.getValue(constant));
        }
        return result;
    }
