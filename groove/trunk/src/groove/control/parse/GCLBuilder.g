...
@init{ ControlState start = builder.getStart(); ControlState end = builder.getEnd(); ControlState newState; ControlTransition fail; }
	...
	| ^(PLUS expression { builder.restore(end,end); } expression)
	| ^(STAR { newState = builder.newState(); builder.restore(start,newState); builder.addLambda(); builder.restore(newState,newState); } expression { builder.restore(newState,end); builder.addLambda(); builder.tagDelta(start);} )
	| ^(SHARP { fail = builder.addElse(); builder.restore(start, start); } expression { builder.fail(start,fail); })
	| ^(CALL IDENTIFIER) { proc(builder.getProc($IDENTIFIER.text)); }
	| 'true' { builder.addLambda(); builder.tagDelta(start); }
	| rule
	; 
