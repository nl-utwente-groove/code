 ...
 * @version $Revision $
 */
public class ReteMatchStrategy extends MatchStrategy<TreeMatch> {
    /**
     ...
     */
    public ReteMatchStrategy(ReteSearchEngine owner, Condition condition) {
        this.owner = owner;
    ...
    private TreeMatch createTreeMatch(ReteMatch matchMap, HostGraph host) {
        ...
        ReteMatchStrategy[] subMatchers = getSubMatchers();
     ...
     */
    private ReteMatchStrategy[] getSubMatchers() {
        if (this.subMatchers == null) {
            List<ReteMatchStrategy> result =
                new ArrayList<ReteMatchStrategy>(
            ...
            for (Condition subCondition : this.condition.getSubConditions()) {
                if (!(subCondition instanceof EdgeEmbargo)) {
                    result.add(new ReteMatchStrategy(this.owner, subCondition));
                }
            }
            this.subMatchers =
                result.toArray(new ReteMatchStrategy[result.size()]);
        }
    ...
    private ReteMatchStrategy[] subMatchers;
}
