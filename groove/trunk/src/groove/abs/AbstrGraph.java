...
public interface AbstrGraph extends Graph {
     ...
     * @param p
     * @return The unique node having type <code>p</code>, or null if this
     *         type is not present in the graph.
     */
    ...
    /**
     * Ensures that a node for the pattern is represented in the graph, and adds
     * q to its multiplicity.
     * @return <code>true</code> if the pattern <code>p</code> existed in
     *         the abstract graph and had multiplicity zero
     */
    ...
    /**
     * Checks whether two abstract graphs are isomorphic. Let G1 and G2 be two
     * abstract graphs, and consider the mapping map : G1.nodeSet() ->
     * G2.nodeSet() map(n1) = n2 iff n1 and n2 have the same type. Then G1 and
     * G2 are isomorphic if map defines an isomorphism on their graph structure,
     * map respects multiplicity, and G1 and G2 have types on the same pattern
     * family.
     * @param other
     * @return An isomorphism between this graph and other, or null if the
     *         isomorphism does not exist.
     */
    public Morphism getIsomorphismToAbstrGraph(AbstrGraph other);

    /**
     * Tests equality of abstract graphs. Two abstract graphs are equal if they
     * have isomorphic structure preserving types and multiplicities, are
     * constructed over the same family and with the same precision constraint.
     * @param o
     * @return <code>true</code> if o is an abstract graph and is equal to
     *         this abstract graph
     */
     ...
     * family and precision for both graphs are required. An abstract graph G1
     * is included into an abstract graph G2 if all multiplicity sets of G1 are
     * included into G2.
     * @param other
    ...
    /**
     * Returns true when the abstract graph does not have concretisations. If
     * this method returns false, this does not mean that the abstract graph
     * admits concretisations.
     * @return true if it can be determined that the abstract graph does not
     *         admit concretisations, false if this cannot be determined
     */
