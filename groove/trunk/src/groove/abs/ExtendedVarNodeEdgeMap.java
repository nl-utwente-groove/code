...
// Contains an origin map (VarNodeEdgeMap) together with a pre-processed
// information on the number of pre-images of all nodes in the codomain

/**
 * Extends a VarNodeEdgeMap with a the possibility to retrieve the number of
 * pre-images for nodes
 * 
 * @author Iovka Boneva
 * @version $Revision $
 */
...
class ExtendedVarNodeEdgeMap extends VarNodeEdgeHashMap {
    ...
    public ExtendedVarNodeEdgeMap(NodeEdgeMap map) {
        ...
        this.nbPI = new HashMap<Node,Integer>(nodeMap().size());
    ...
    /**
     * The number of pre-images of a given node. Lazy computation of the
     * pre-images map.
     * @param n
    ...
    final int getNbPreIm(Node n) {
        assert nodeMap().containsValue(n) : "Node " + n
            + " not in co-domain of " + super.toString();
