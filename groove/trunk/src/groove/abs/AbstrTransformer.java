...
public class AbstrTransformer {
	...
	public static RuleEvent transform(final AbstrGraph host, 
			                          ...
			                          Collection<AbstrGraph> result) {
		// Compute the possible concrete parts
		ConcretePart.Typing typing = new ConcretePart.Typing() {
			public GraphPattern typeOf(Node n) { return host.typeOf(match.getElementMap().getNode(n)); }
		};
		...
		for (ConcretePart cp : ext) {
			...
			Collection<AbstrGraph> transformations = smat.transform(appl, nodeFactory);
		...
		RuleEvent transitionEvent = new SPOEvent(match.getRule(), match.getElementMap(), nodeFactory, false);
