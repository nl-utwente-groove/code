...
public class ScenarioChecker {
    ...
    public static final Class<?> STRATEGY_CLASS =
        groove.explore.strategy.Strategy.class;
    /** The super-class of all results. */
    public static final Class<?> RESULT_CLASS =
        groove.explore.result.Result.class;
    /** The super-class of all acceptors. */
    public static final Class<?> ACCEPTOR_CLASS =
        groove.explore.result.Acceptor.class;

    /** A list of packages where strategies should be looked for by default. */
    public static final String[] STRATEGY_PACKAGES =
        {"groove.explore.strategy"};
    ...
    /**
     * Gets a class object from a string description. The given name may be
     * either relative to one of the strategy/result/acceptor packages, or
     * absolute.
     ...
     */
    public static Class<?> getClass(String name, Component comp)
        throws ClassNotFoundException {
        switch (comp) {
        case STRATEGY:
            return getClass(name, STRATEGY_PACKAGES);
        case RESULT:
            return getClass(name, RESULT_PACKAGES);
        case ACCEPTOR:
            return getClass(name, ACCEPTOR_PACKAGES);
        default:
    ...
    /** Helper method */
    private static Class<?> getClass(String name, String[] packages)
        throws ClassNotFoundException {
        if (name.contains(".")) {
    ...
    public <T> boolean isAllowed(Strategy str, Result res, Acceptor acc) {
        return this.allowed.isAllowedConfiguration(str.getClass(),
            res.getClass(), acc.getClass());
    }

    public boolean isAllowed(Class<?> strClass, Class<?> resClass,
            Class<?> accClass) {
    ...
    public ScenarioChecker() {
        ...
        try {
            ...
            InputStream inStream = configURL.openStream(); // IOException
                                                           // possible
            ...
            System.err.println("Could not open configuration file "
                + Groove.ALLOWED_SCENARIOS_CONFIGURATION_FILE);
            e.printStackTrace();
        }
        ...
        try {
            ...
            e.printStackTrace();
            System.err.println("Please correct upon errors in the configuration file ("
                + Groove.ALLOWED_SCENARIOS_CONFIGURATION_FILE + ").");
