...
/**
 * Extension of a map from rules to collections of a generic type,
 * that additionally keeps track of the combined size of the collections.
 * The total size is only accurate if the collections are not modified after addition.
...
public class MatchMap<T> extends LinkedHashMap<Rule, Collection<T>> {
    ...
    public Iterator<T> globalIterator() {
    	Iterator<Iterator<T>> iter = new TransformIterator<Collection<T>,Iterator<T>>(values().iterator()) {
			@Override
