...
/**
 * Strategy that closes every state it explores, and adds the newly generated states to
 * a pool, together with information regarding the outgoing transitions of its parent.
 * Subclasses must decide on the order of the pool; e.g., breadth-first or depth-first.
 */
abstract public class ClosingStrategy extends AbstractStrategy {
	public boolean next() {
		...
		Collection<RuleEvent> matchSet = createMatchCollector(cache).getMatchSet();
		...
		while (matchIter.hasNext()) {
			...
			GraphTransition trans = getMatchApplier().addTransition(getAtState(), nextEvent, cache.getTarget(nextEvent.getRule()));
     ...
     * @param newVirtualEvents the virtual events generated for the current state
     */
	...
	/** 
	 * This collector takes the virtual events for the current state into account.
	 */
	@Override
	protected MatchSetCollector createMatchCollector(ExploreCache cache) {
	    return new MatchSetCollector(getAtState(), cache, getRecord(), virtualEvents);
	}
	...
	protected static class PoolElement extends Pair<GraphState,Collection<VirtualEvent.GraphState>> {
	    /** Constructs a pool element from a given state and virtual event set. */
        public PoolElement(GraphState state, Collection<groove.trans.VirtualEvent.GraphState> virtualEvents) {
