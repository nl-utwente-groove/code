...
public class STS {
    ...
    private final Map<GeneralizedGraph,Location> locationMap;
    private final Map<Object,SwitchRelation> switchRelationMap;
    private final Set<Gate> gates;
    // not injective
    private final Map<Pair<Integer,TypeLabel>,LocationVariable> locationVariables;
    // injective
    private final Map<Pair<VariableNode,Rule>,InteractionVariable> interactionVariables;
     ...
     * 
     * @param graph
     *            The graph by which the location is represented.
     * @return The location.
     */
    public Location getLocation(GeneralizedGraph graph) {
        return this.locationMap.get(graph);
    }
    ...
    public SwitchRelation ruleMatchToSwitchRelation(HostGraph sourceGraph,
        ...
        for (int i = 0; i < end; i++) {
            ...
            if (k instanceof VariableNode) {
                ...
                System.out.println("ERROR: non-variable node " + k.toString()
                    + " listed as parameter");
            }
        }
        ...
        for (RuleEdge le : lhs.edgeSet()) {
            if (le.getType() != null && le.target() instanceof VariableNode) {
                ...
                if (var == null && !isFinal(sourceGraph, hostEdge.source())) {
                    System.out.println("ERROR: Data node found not mapped by any variable: "
                        + hostEdge);
        ...
        for (RuleEdge creatorEdge : rule.getCreatorEdges()) {
            if (creatorEdge.target().getType().isDataType()) {
                ...
                if (eraserEdge == null) {
                    ...
                    System.out.println("ERROR: no eraser edge found for created location variable "
                        + creatorEdge
                ...
                if (var == null) {
                    ...
                    System.out.println("ERROR: no location variable found referenced by "
                ...
                if (updateValue.length() == 0) {
                    ...
                    System.out.println("ERROR: Update of " + var.toString()
                ...
                if (resultType != ((VariableNode) eraserEdge.target()).getSignature()) {
                    ...
                    System.out.println("ERROR: The result type of the expression "
                        + updateValue
                        + " ("
                        + resultType
    ...
    public String toJSON() {
        String json =
            "{\"_json\":{\"start\":" + this.start.toJSON() + ",\"lVars\":{";
