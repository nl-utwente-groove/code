...
public class TestingMultiplicities extends TestCase {
    ...
    private static final Multiplicity M = Abstraction.MULTIPLICITY; // a
                                                                    // shortcut

    // the values among which to test
    private static final Set<MultiplicityInformation> zero, one, two, omega,
            omega_two, omega_two_one, omega_two_one_zero;
    ...
    static {
        ...
        omega_two.addAll(omega);
        omega_two.addAll(two);
        omega_two_one.addAll(omega_two);
        omega_two_one.addAll(one);
        omega_two_one_zero.addAll(omega_two_one);
        omega_two_one_zero.addAll(zero);
        ...
        all.add(omega);
        all.add(omega_two);
        all.add(omega_two_one);
        all.add(omega_two_one_zero);
    }

    /** */
    public void testOperations() throws ExceptionRemovalImpossible,
        AssertionError {
    ...
    public void testCompare() throws ExceptionRemovalImpossible, AssertionError {
        ...
        for (int i = 0; i < T.length; i++) {
            for (int j = 0; j < T.length; j++) {
                if (i == j) {
                    assertEquals(M.compare(T[i], T[j]),
                        MultInfoRelation.M_EQUAL);
                } else {
                    assertEquals(M.compare(T[i], T[j]),
                        MultInfoRelation.M_NOTEQUAL);
                }
        ...
        for (int i = 0; i < T.length - 1; i++) {
            assertEquals(M.compare(T[i], zero_one_two_omega),
                MultInfoRelation.M_BELONGS);
            assertEquals(M.compare(zero_one_two_omega, T[i]),
                MultInfoRelation.M_CONTAINS);
        }
        assertEquals(M.compare(T[3], zero_one_two_omega),
            MultInfoRelation.M_SUBSET);

        assertEquals(M.compare(one_two_omega, zero_one_two_omega),
            MultInfoRelation.M_SUBSET);

        assertEquals(M.compare(zero_one_two_omega, one_two_omega),
            MultInfoRelation.M_SUPERSET);

        assertEquals(M.compare(two_omega, M.remove(T[3], 1)),
            MultInfoRelation.M_EQUAL);
    }
    ...
    private Set<MultiplicityInformation> arrayToSet(
            MultiplicityInformation[] array) {
        Set<MultiplicityInformation> result =
            new HashSet<MultiplicityInformation>(4);
