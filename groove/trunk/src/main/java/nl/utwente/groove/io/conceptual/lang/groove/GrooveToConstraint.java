...
import nl.utwente.groove.io.conceptual.configuration.schema.TypeModel.Constraints;
import nl.utwente.groove.io.conceptual.graph.AbsEdge;
import nl.utwente.groove.io.conceptual.graph.AbsGraph;
import nl.utwente.groove.io.conceptual.graph.AbsNode;
import nl.utwente.groove.io.conceptual.lang.Message;
import nl.utwente.groove.io.conceptual.lang.Messenger;
...
public class GrooveToConstraint implements Messenger {
    ...
    private void parseRules() {
        ...
        for (RuleModel model : this.m_ruleModels) {
            ...
            if (constraints.isCheckUniqueness()
                ...
                && lastName.startsWith(ConstraintToGroove.DEFAULT_PRF)) {
        ...
        for (AbsNode n : ruleGraph.getNodes()) {
            ...
            if (n.getTargetEdges().size() == 0 && getType(n) != null) {
                ...
                if (n.getEdges().size() > 0) {
                    ...
                    if (c.getField(fieldName).getType() instanceof Container con) {
                        con.setUnique(true);
                    }
                }
            }
    ...
    private void parseIdentityRule(RuleModel model) {
        ...
        if (classNode != null) {
            ...
            if (fieldNames.size() > 0) {
                IdentityProperty ip = new IdentityProperty(idClass,
    ...
    private Value getNodeValue(AbsNode node) {
        ...
        // Custom data type
        else if (nodeType instanceof CustomDataType cdt) {
        ...
        // regular data types
        else if (nodeType instanceof DataType dt) {
            String valueString = node.getNames()[0].substring(nodeType.typeString().length() + 1);
            resultValue = dt.valueFromString(valueString);
        }
        // Containers & tuples
