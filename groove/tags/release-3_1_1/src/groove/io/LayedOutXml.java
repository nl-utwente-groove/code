...
public class LayedOutXml extends AbstractXml implements Xml<Graph> {
    ...
    private void marshalGraph(Graph graph, LayoutMap<Node,Edge> layoutMap,
            File file) throws IOException {
        // first marshal the graph
        this.marshaller.marshalGraph(graph, file);
        // if there is layout information, create a file for it
        PrintWriter layoutWriter =
            new PrintWriter(new FileWriter(toLayoutFile(file)));
    ...
    /**
     * Reads the layout information for a given graph from a given reader. Any
     * errors in the layout information are added to the graph errors.
     ...
     */
    private LayoutMap<Node,Edge> readLayout(Graph graph,
            Map<String,Node> nodeMap, BufferedReader layoutReader)
        throws IOException {
                ...
                try {
                    ...
                    if (parts.length > 0) {
                        ...
                        if (command.equals(NODE_PREFIX)) {
                            ...
                            Edge edge =
                                putEdgeLayout(layoutMap, parts, nodeMap,
                                    version);
                            if (!graph.containsElement(edge)) {
                                throw new FormatException("Unknown edge %s",
                                    edge);
                            }
     ...
     */
    protected void putVertexLayout(LayoutMap<Node,Edge> layoutMap,
     ...
     */
    protected Edge putEdgeLayout(LayoutMap<Node,Edge> layoutMap,
            String[] parts, Map<String,Node> nodeMap, int version)
     ...
     */
    protected Point2D calculateLabelPosition(Point2D label,
