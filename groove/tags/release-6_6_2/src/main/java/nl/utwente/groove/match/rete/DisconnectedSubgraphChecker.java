...
public class DisconnectedSubgraphChecker extends ReteNetworkNode implements ReteStateSubscriber {
    ...
    protected List<AbstractReteMatch> makeWholeMatchesIfPossible(ReteNetworkNode antecedent,
        ...
        int jumpIndex = this.getAntecedents()
            .indexOf(antecedent) + repeatIndex;
        ...
        if (isPossible) {
            ...
            Iterator<AbstractReteMatch>[] partialMatchIterators = new Iterator[this.getAntecedents()
                .size()];
            for (int i = 0; i < partialMatchIterators.length; i++) {
                if (i != jumpIndex) {
                    partialMatchIterators[i] = getPartialMatchesFor(this.getAntecedents()
                        .get(i)).iterator();
                }
            }
            ...
            int j = 0;
            boolean injective = this.getOwner()
                .isInjective();
            while (j >= 0) {
                        ...
                        if (partialMatchIterators[j] != null) {
                            if (!partialMatchIterators[j].hasNext()) {
                                partialMatchIterators[j] =
                                    getPartialMatchesFor(this.getAntecedents()
                                        .get(j)).iterator();
                                j--;
                            } else {
                                break;
