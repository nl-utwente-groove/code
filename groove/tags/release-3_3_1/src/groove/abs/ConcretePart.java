...
/**
 * Contains all algorithms used for constructing the graph structure of a
 * concrete part of a materialisation.
 * @author Iovka Boneva
 * @version $Revision $
 */
public class ConcretePart {
     ...
     * @throws ExceptionIncompatibleWithMaxIncidence when some of the
     *         neighbourhoods is not compatible with the family
     */
    private void initNeigh(PatternFamily family)
        throws ExceptionIncompatibleWithMaxIncidence {
        assert this.neigh == null : "Double initialisation of neighbourhoods.";
        checkInvariants();
        this.neigh =
            new HashMap<Node,Graph>(this.graph.nodeSet().size()
                - this.type.size());
        for (Node n : this.graph.nodeSet()) {
            if (!this.centerNodes.contains(n)) {
                // ExceptionIncompatibleWithMaxIncidence possible when the
                // neighbourhood has become too large
    ...
    /**
     * Computes the set of ConcretePart that extend a graph with neighbourhoods
     * for all its nodes.
     ...
     * @param symmetryReduction indicates whether symmetry reduction is to be
     *        performed while computing typing morphisms
     * @return The set of extensions of <code>center</code>
     * @require typing.get(n) != null for all n in center.nodeSet()
     */
    public static Collection<ConcretePart> extensions(Graph center,
            Typing typing, PatternFamily family, boolean symmetryReduction,
            NodeFactory nodeFactory) {
        ArrayList<ConcretePart> result = new ArrayList<ConcretePart>();
        ArrayList<ConcretePart> temp;

        ConcretePart base =
            new ConcretePart(center, new HashMap<Node,Morphism>(),
                new HashSet<Node>(center.nodeSet()));
        result.add(base);
        for (Node n : center.nodeSet()) {
            temp = new ArrayList<ConcretePart>();
            for (ConcretePart cp : result) {
                temp.addAll(cp.extension(n, typing.typeOf(n), null, family,
                    symmetryReduction, nodeFactory));
            }
            result = temp;
        }
    ...
    /**
     * Computes the set of ConcretePart that extend a graph with neighbourhoods
     * for some of its nodes. This method takes into account pre-computed
     * neighbourhood typings for some nodes.
     ...
     * @param typing Gives the desired type and typing morphism for all nodes in
     *        <code>center</code>.
     * @param family The pattern family from which types come
     * @param symmetryReduction indicates whether symmetry reduction is to be
     *        performed while computing typing morphisms
     * @return The set of extensions of <code>center</code>
     * @require typing.get(n) != null for all n in center.nodeSet() OPTIM this
     *          method is not optimal, as it computes concrete parts first
     */
    static Collection<Graph> extensions(ConcretePart concrPart,
            Collection<Node> nodes, SubTyping typing, PatternFamily family,
            boolean symmetryReduction, NodeFactory nodeFactory) {
        ArrayList<ConcretePart> concrParts = new ArrayList<ConcretePart>();
        ArrayList<ConcretePart> temp;

        ConcretePart base =
            new ConcretePart(concrPart.graph(), new HashMap<Node,Morphism>(),
                concrPart.centerNodes);
        concrParts.add(base);
        for (Node n : nodes) {
            temp = new ArrayList<ConcretePart>();
            for (ConcretePart cp : concrParts) {
                assert typing.typeMapOf(n) != null : "Something's wrong.";
                temp.addAll(cp.extension(n, typing.typeOf(n),
                    typing.typeMapOf(n), family, symmetryReduction, nodeFactory));
            }
            concrParts = temp;
        }
    ...
    /**
     * Constructs the extensions of the concrete part by ensuring a complete
     * neighbourhoods of the node <code>n</code> w.r.t. its desired type
     * <code>typeN</code>, and eventually with pre-computed typing of the
     * existing neighbourhood.
     * @param n (final) the node whose possible neighbourhoods are constructed
     * @param typeN (final) the desired type of the node n
     * @param preTyping (final) The typing from part of the neighbourhood of n
     *        into typeN. If null, then all such matchings should be considered.
     * @param symmetryReduction indicates whether symmetry reduction is to be
     *        performed while computing typing morphisms
     * @return the set of all possible extensions of the concrete part
     * @require n is a node in this.graph()
     * @ensure all concrete parts in the result have this.concrPart as common
     *         subgraph
     */
    private Collection<ConcretePart> extension(Node n, GraphPattern typeN,
            NodeEdgeMap preTyping, PatternFamily family,
            boolean symmetryReduction, NodeFactory nodeFactory) {
        ...
        for (NodeEdgeMap m : typeN.possibleTypings(neighN, baseMatch,
            symmetryReduction)) {
            result.addAll(extensionsByMorphism(m, typeN, n, family, nodeFactory));
        }
    ...
    /**
     * Computes all possible extensions of the concrete part by preserving a
     * morphism centered in a node. Extensions are obtained by adding the
     * codomain of the morphism, and trying to merge the newly added nodes with
     * existing ones in all possible ways so that existing neighbourhoods are
     * preserved. Used by extension(Node, GraphPattern)
     * @param morph a map representing a morphism from the neighbourhood of
     *        <code>node</code> into its type.
     ...
     * @see #extension(Node, GraphPattern, NodeEdgeMap, PatternFamily, boolean,
     *      NodeFactory)
     */
    private Collection<ConcretePart> extensionsByMorphism(NodeEdgeMap morph,
            Graph cod, Node node, PatternFamily family, NodeFactory nodeFactory) {
        checkInvariants();
        ...
        for (Node n : baseGraph.nodeSet()) {
            if (baseMorphMap.containsKey(n)) {
                if (!this.graph.containsElement(n)) {
                    mergeableNodesB.add(n);
                }
            } else {
                mergeableNodesA.add(n);
            }
        }
        // any of mergeableNodesB can be merged with any of mergeableNodesA (or
        // with none)
        // no repetitions allowed (i.e. if x,y \in B, x merged with a, y merged
        // with b, then a != b)
        ...
        // allowed to obtain resGraphs[i] from baseGraph
        ...
        // postcond : resGraphs contains graphs with all possible mergings of
        // already iterated nodes (including no merging of iterated nodes)
        for (Node n : mergeableNodesB) {
            ...
            // merging node n with any of the unused nodes for merging, this for
        ...
        // now the resXXX tables contain enough information for constructing the
        // resulting concrete parts
        ArrayList<ConcretePart> result = new ArrayList<ConcretePart>();
        for (int i = 0; i < resGraphs.size(); i++) {
            HashMap<Node,Morphism> newCpType =
                new HashMap<Node,Morphism>(this.type);
            // construct the morphism corresponding to the resMorphisms(i) map
            Morphism resCpMorph = null;
            try {
                resCpMorph =
                    new DefaultMorphism(family.getNeighInGraph(
                        resGraphs.get(i), node), cod, resMorphisms.get(i));
            } catch (ExceptionIncompatibleWithMaxIncidence exc) {
                // If exception, then the actually constructed extension is
                // incorrect (the neighbourhood has become bigger that allowed,
                // thus than its type)
                continue;
            }

            // Check that the neighbourhood has not became bigger than its type
            Collection<VarNodeEdgeMap> extensions =
                Util.getMatchSet(resCpMorph.dom(), resCpMorph.cod(),
                    resMorphisms.get(i));
            if (extensions.isEmpty()) {
                // the currently constructed extension is incorrect (the
                // neighbourhood has become bigger than its type)
                continue;
            }
            assert extensions.size() == 1 : "Too many extensions";
            newCpType.put(node, resCpMorph);
            ConcretePart resCP =
                new ConcretePart(resGraphs.get(i), newCpType, this.centerNodes);
    ...
    /**
     * Checks whether this concrete part is correct. A concrete part is correct
     * when the neighbourhood of all typed node complies with its type. The
     * unique source of incorrectness considered is the presence of additional
     * edges.
    ...
    /**
     * Associates with nodes their neighbourhood. Initialised after completion
     * of the graph part.
     */
    private Map<Node,Graph> neigh;
    /**
     * Associates typing morphisms with some nodes of the graph. Each such
     * morphism is from the neighbourhood of the node into its type pattern.
     * Used only during the computation.
     */
    private Map<Node,Morphism> type;
    /**
     * The set of center nodes.
     * @invariant (A) whenever neigh != null, neigh.keySet() union centerNodes
     *            is graph.nodeSet()
     */
    private final Set<Node> centerNodes;
     ...
     */
    private ConcretePart(Graph graph, Map<Node,Morphism> type,
            Set<Node> centerNodes) {
        this.graph = graph;
        this.graph.setFixed();
        this.type = type;
    ...
    /**
     * The set of center nodes of the concrete part. These are exactly the nodes
     * for which the concrete part has full neighbourhood (thus, the
     * neighbourhood mapping does not contain these nodes as keys).
     * @return The set of center nodes of the concrete part.
     */
    final Set<Node> centerNodes() {
