...
public class SingularIterator<T> implements Iterator<T> {
    ...
    public boolean hasNext() {
        return !this.thisDelivered;
    }

    /**
     * Yields the inner object if that has not yet been delivered (as indicated
     * also by {@link #hasNext()}.
     * @throws NoSuchElementException if the inner object has been delivered
     *         before
     */
    public T next() {
        if (this.thisDelivered) {
            throw new NoSuchElementException();
        } else {
            this.thisDelivered = true;
            return this.obj;
        }
    }

    /**
     * This iterator supports removal, but nothing happens since there is no
     * underlying set.
     */
    public void remove() {
        if (!this.thisDelivered) {
