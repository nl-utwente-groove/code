...
/**
 * Tree adaptor creating {@link CtrlTree} nodes and error nodes. 
 * @author Arend Rensink
 * @version $Revision $
 */
public class CtrlTreeAdaptor extends CommonTreeAdaptor {
    @Override
    public CtrlTree create(Token payload) {
        return new CtrlTree(payload);
    }

    @Override
    public CtrlTree errorNode(TokenStream input, Token start, Token stop,
            RecognitionException e) {
        return new MyErrorNode(input, start, stop, e);
    }

    /** Creates a tree node stream based on this adaptor. */
    public TreeNodeStream createTreeNodeStream(CtrlTree tree) {
    ...
    /**
     * Subclass of {@link CtrlTree} that delegates to {@link CommonErrorNode}. This is
     * necessary to ensure that all tree nodes are {@link CtrlTree} instances.
     */
    private class MyErrorNode extends CtrlTree {
