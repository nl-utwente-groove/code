 ...
 *
 * $Id: BehaviourOption.java,v 1.2 2007-08-26 07:24:01 rensink Exp $
 */
package groove.gui;
 ...
 * or <i>Ask</i>, <i>Always</i> or <i>Never</i>. This refers to the setting for
 * some behavioural feature of the tool.
 */
public class BehaviourOption extends JMenu implements ItemListener {
	...
	public BehaviourOption(String name, int choiceCount) {
		...
		answers = choiceCount == 2 ? standardAnswers2 : standardAnswers3;
	...
	/** Sets the menu value to one of {@link #ASK}, {@link #ALWAYS} or {@link #NEVER}. */
	...
	/**
	 * Indicates if this option is confirmed.
	 * This can be either because its value is {@link #ALWAYS}, or because
	 * the user has confirmed an appropriate dialog. 
	 * @param owner the component on which the dialog is to be shown
	 * @param question if not <code>null</code>, replaces the name of the option in the dialog
	...
	public boolean confirm(Component owner, String question) {
		if (value == ASK) {
			...
			JOptionPane pane = new JOptionPane(question, JOptionPane.QUESTION_MESSAGE, JOptionPane.YES_NO_CANCEL_OPTION, null, options.toArray());
	...
	private List<String> getDialogOptions() {
		if (dialogOptions == null) {
			dialogOptions = new ArrayList<String>();
			dialogOptions.add(YES_TEXT);
			dialogOptions.add(NO_TEXT);
			Iterator<String> answerIter = getAnswers().iterator();
			answerIter.next();
			while (answerIter.hasNext()) {
				dialogOptions.add(answerIter.next());
			}
		}
	...
	/** Value belonging to the <i>Always</i> (or <i>Auto</i>) choice of the menu. */
	...
	static private final List<String> standardAnswers2 = Collections.unmodifiableList(Arrays.asList( ASK_TEXT, ALWAYS_TEXT ));
