...
public interface DeltaApplier {
	 ...
	 * methods on a given target.
	 * The effect is the same as calling {@link #applyDelta(DeltaTarget, int)} with {@link #ALL_ELEMENTS}
	 * as second parameter.
	 ...
	 * {@link DeltaTarget#removeNode(Node)} and the corresponding <code>Edge</code>
	 * methods on a given target.
	 * A second parameter controls whether nodes, edges or both should be added and removed.
	 * @param target the target to be processed
	 * @param mode if {@link #NODES_ONLY}, only nodes will be processed; if {@link #EDGES_ONLY},
