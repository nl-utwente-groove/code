...
public class ExprParser {
     ...
     * @param expr the string to be parsed
     * @return a pair of objects: the first is the string with all
     * quoted and bracketed substrings replaced by the character {@link #PLACEHOLDER},
     * and the second is a list of the replaced substrings, in the order in which
     * they appeared in the original string.  
     */
    ...
    static public String toTrimmed(String expr, char open, char close) throws FormatException {
        ...
        if (parseResult.first().length() != 1 || parseResult.second().get(0).charAt(0) != open) {
            throw new FormatException("Expression %s not surrounded by bracket pair %c%c", expr, open, close);
        } else {
     ...
     * The {@link #ESCAPE_CHAR} itself is by default also escaped.
     ...
     * The original string does not need to be quoted.
     * @param string the original string
     * @param quote the quote character to be used
     * @boolean test if <code>true</code>, the method throws an exception upon finding a format
     * error; otherwise, it returns <code>null</code>
     * @return the unquoted string (which may equal the original, if there are no quotes or
     * escaped characters in the string), or <code>null</code> if <code>test</code> is <code>false</code>
     * and there is a format error.
     * @throws FormatException if there are unescaped quotes at any position except the first or
     * last, or if there are no matching begin or end quotes
     */
    ...
    /** Tests if a character may occur as the first character in a wildcard identifier. */
     ...
     * starts with a correct character (according to {@link #isIdentifierStartChar(char)}),
     * and contains only characters satisfying {@link #isIdentifierChar(char)}.
    ...
    /**
     * The characters allowed at the start of a wildcard identifier, apart from letters.
     * @see ExprParser#isIdentifier(String)
     */
