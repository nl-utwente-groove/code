...
public class ReteTool extends GrooveCmdLineTool<Object> {
    ...
    protected Object run() throws Exception {
        ...
        emit("RETE network shape for %s was successfully saved to %s ", getGrammarDir(),
             outFileName);
        return null;
    }

    private String doSaveReteNetwork() throws IOException, FormatException {
        GrammarModel grammar = SystemStore.newGrammar(getGrammarDir());
        String name = grammar.getName() + ".rete";
        String filePath = hasOutFileName()
            ? getOutFileName()
            : name;
        new ReteSearchEngine(grammar.toGrammar()).getNetwork().save(filePath, name);
