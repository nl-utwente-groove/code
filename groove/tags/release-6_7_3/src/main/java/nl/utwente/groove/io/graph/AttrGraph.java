...
public class AttrGraph extends NodeSetEdgeSetGraph<AttrNode,AttrEdge> {
     ...
     * The node number is generated by the {@link NodeNrDispenser} instance.
    ...
    public AttrNode addNode(String id) {
        ...
        AttrNode node;
        boolean nodeIdTooHigh = nodeNr > MAX_NODE_NR;
        if (nodeIdTooHigh) {
        ...
        if (nodeIdTooHigh) {
            addError("Stored node ID %s is likely to cause memory overflow. "
                + "Change node IDs in source or set grammar property 'Use stored node IDs' to 'false'",
                     id, result);
        }
    ...
    /** Maximum stored node number that will be accepted without complaints. */
    static public final int MAX_NODE_NR = 10_000_000;
